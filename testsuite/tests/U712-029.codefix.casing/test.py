...
def run():
    ...
    yield wait_tasks()
    gps_assert(len(GPS.Message.list()),
               6,
               "Missing messages for bad casing")
    gps_assert(GPS.MDI.get("foo.adb"),
               None,
               "foo.adb should not be opened")
    gps_assert(GPS.MDI.get("bar.adb"),
               None,
               "bar.adb should bot be opened")
    gps_assert(GPS.MDI.get("bar.ads"),
               None,
               "bar.ads should bot be opened")
