    ...
    def probe(self, testsuite, dirpath, dirnames, filenames):
        for k in self.dirs:
            if dirpath.startswith(k):
                return super(GSTestFinder, self).probe(
                    testsuite, dirpath, dirnames, filenames
                )
        return None
...
class GSPublicTestsuite(Testsuite):
    def add_options(self, parser):
        parser.add_argument(
            "--noxvfb", default=False, action="store_true", help="disable Xvfb"
        )
        parser.add_argument(
            "--valgrind_memcheck",
            action="store_true",
    ...
    def set_up(self):
        ...
        os.environ["ADA_DEBUG_FILE"] = os.path.join(self.test_dir, "tests", "gnatdebug")
        ...
        if self.env.options.valgrind_memcheck:
            self.env.valgrind_cmd = [find_executable("valgrind")] + [
                opt.format(base=base) for opt in VALGRIND_OPTIONS
            ]
