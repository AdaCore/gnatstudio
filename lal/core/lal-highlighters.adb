...
package body LAL.Highlighters is
   ...
   package Highlights_Holders is
      ...
      with
        Static_Predicate =>
          Style_Subset
          in None | Aspect | Block | Type_Style | Aspect_Block | Aspect_Type;
      ...
      with
        Pre =>
          not Libadalang.Common.Is_Trivia (From)
          and then not Libadalang.Common.Is_Trivia (To);
   ...
   package body Highlights_Holders is
      ...
      begin
         ...
         if Libadalang.Common.No_Token in First | Last or else Last < First
         then
            Self.First := 0;
            Self.Vector.Clear;
            Empty := True;
   ...
   is
      ...
      Aspect_Prefix :
        constant array (None .. Type_Style, Boolean) of Style_Enum_Type :=
          (None         => (None, Aspect),
   ...
   begin
      ...
      case Token is
         ...
         when Ada_Termination | Ada_Lexing_Failure | Ada_Whitespace =>
            return Aspect_Prefix (None, In_Aspect);
   ...
   is
      ...
      procedure Highlight_Name_List
        (List : Alternatives_List; Style : Style_Enum_Type);
      ...
      From_Token : constant Token_Reference :=
        (if From /= -1
         then Unit.Lookup_Token ((Line_Number (From), 1))
         else Unit.First_Token);

      To_Token : constant Token_Reference :=
        (if To /= -1
         then Unit.Lookup_Token ((Line_Number (To + 1), 1))
      ...
      procedure Highlight_Name_List
        (List : Alternatives_List; Style : Style_Enum_Type) is
      begin
            ...
            declare
               Is_Keyword : constant Boolean :=
                 Libadalang.Analysis.Is_Keyword
                   (Token => Token, Version => Libadalang.Common.Ada_2012);
            ...
            begin
               ...
               Self.Apply_Style
                 (Token => Token,
                  Style =>
                    To_Style
                      (Kind (Data (Token)), False, In_Aspect, Wrong_Literal));
      ...
      begin
         ...
         for J of Node.Parents loop
            if Is_Ghost_Root_Node (J) or else Node.Kind = Ada_Aspect_Assoc then
               return True;
            end if;
         end loop;
   ...
   begin
      ...
      declare
         First_Node : constant Ada_Node :=
           (if From /= -1
            then Root.Lookup ((Line_Number (From), 1))
            else Root);
   ...
   begin
      for J of Known_Styles loop
         Self.Remove_Highlighting (J.all, From, To);
      end loop;
   ...
   begin
      Self.Highlight_Token (Token => Token, Style => Get_Known_Style (Style));
   end Apply_Style;
