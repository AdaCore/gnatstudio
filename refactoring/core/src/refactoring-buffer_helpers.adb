...
package body Refactoring.Buffer_Helpers is
   ...
   begin
      return Editor.New_Location (Get_Line (Location), Get_Column (Location));
   end To_Location;
   ...
   begin
      Trace
        (Me,
         "COMMENT CODE: "
         & Get_Line (Start)'Img
         & Get_Column (Start)'Img
         & " TO "
         & Get_Line (Stop)'Img
         & Get_Column (Stop)'Img);
      Line_Cursor := To_Location (Get_File (Start), Get_Line (Start), 1);
      ...
      if Get_Line (Start) = Get_Line (Stop) then
         ...
         begin
            ...
            if not Is_Blank_Line (Back) then
               ...
               Tmp_Cursor :=
                 To_Location (Get_File (Stop), Get_Line (Stop) + 1, 1);
   ...
   begin
      if Get_Line (Location) = 0 then
         ...
         Set_Column
           (Insert_Position'Access, Get_Column (Insert_Position'Access) + 1);
      ...
      if Indent then
         ...
         begin
            Set_Line_Column
              (Line_Cursor'Access, Get_Line (Line_Cursor'Access) + 1, 1);
   ...
   begin
      declare
         Loc_Start : constant Editor_Location'Class :=
           Editor.New_Location (Get_Line (Location), Get_Column (Location));
      begin
         if Len /= 0 then
            declare
               Loc_End : constant Editor_Location'Class :=
