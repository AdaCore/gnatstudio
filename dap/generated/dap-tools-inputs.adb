...
Reader.Read_Next;
while Success and Reader.Event_Kind /= VSS.JSON.Pull_Readers.End_Array loop
...
Reader.Read_Next;
while Success and Reader.Event_Kind = VSS.JSON.Pull_Readers.Key_Name loop
...
Value.Append ((VSS.JSON.Events.Boolean_Value, Reader.Boolean_Value));
...
ModuleEvent_reason_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
ColumnDescriptor_type_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
StackFrame_presentationHint_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
ExceptionBreakMode_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
StoppedEvent_reason_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
package OutputEvent_category_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["console", "stdout", "stderr", "telemetry"]);
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
OutputEvent_category_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
OutputEvent_group_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
ChecksumAlgorithm_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
ProcessEvent_startMethod_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
Scope_presentationHint_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
CompletionItemType_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
InvalidatedAreas_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
Source_presentationHint_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
LoadedSourceEvent_reason_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
ProtocolMessage_type_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
RunInTerminalRequestArguments_kind_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
VariablesArguments_filter_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
VariablePresentationHint_kind_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
VariablePresentationHint_visibility_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
InitializeRequestArguments_pathFormat_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
ThreadEvent_reason_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
DataBreakpointAccessType_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
BreakpointEvent_reason_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
package EvaluateArguments_context_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["watch", "repl", "hover", "clipboard"]);
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
EvaluateArguments_context_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success : in out Boolean) is
Index : constant Natural := (if Reader.Is_String_Value then
SteppingGranularity_Minimal_Perfect_Hash.Get_Index (Reader.String_Value)else 0);
begin
if Index > 0 then
...
Success := False;
   ...
   when 6 =>  --  message
if Reader.Is_String_Value and then Reader.String_Value = "cancelled" then
Reader.Read_Next;
else
Success := False;
end if;
...
package NextArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["threadId", "granularity"]);
...
package LaunchRequestArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["noDebug", "__restart", "program"]);
...
package AttachRequestArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["__restart"]);
...
package StepOutArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["threadId", "granularity"]);
...
package StepInTarget_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["id", "label"]);
...
package StackTraceArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["threadId", "startFrame", "levels"]);
...
package SourceBreakpoint_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["line", "column"]);
...
package ReverseContinueArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["threadId"]);
...
package DisconnectArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["restart", "terminateDebuggee"]);
...
package StepInArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["threadId", "targetId", "granularity"]);
...
package ContinueArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["threadId"]);
...
package StepBackArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["threadId", "granularity"]);
...
package RunInTerminalRequestArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["kind", "title", "cwd", "args", "env"]);
...
package VariablePresentationHint_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["kind", "attributes", "visibility"]);
...
package CompletionItem_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["label", "text", "sortText", "type", "start", "length", "selectionStart", "selectionLength"]);
...
package DataBreakpointInfoArguments_Minimal_Perfect_Hash is new Minimal_Perfect_Hash (["variablesReference", "name"]);
