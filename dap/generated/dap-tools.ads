...
package DAP.Tools is
   ...
   package Enum is
      ...
      type Optional_StackFrame_presentationHint (Is_Set : Boolean := False)
      is record
         case Is_Set is
            when True =>
      ...
      type OutputEvent_category is (console, stdout, stderr, telemetry);
      ...
      type Optional_ProcessEvent_startMethod (Is_Set : Boolean := False)
      is record
         case Is_Set is
            when True =>
      ...
      type Optional_Scope_presentationHint (Is_Set : Boolean := False)
      is record
         case Is_Set is
            when True =>
      ...
      type Optional_Source_presentationHint (Is_Set : Boolean := False)
      is record
         case Is_Set is
            when True =>
      ...
      type Optional_VariablesArguments_filter (Is_Set : Boolean := False)
      is record
         case Is_Set is
            when True =>
      ...
      type Optional_DataBreakpointAccessType (Is_Set : Boolean := False)
      is record
         case Is_Set is
            when True =>
      ...
      type EvaluateArguments_context is (watch, repl, hover, clipboard);

      type Optional_EvaluateArguments_context (Is_Set : Boolean := False)
      is record
         case Is_Set is
            when True =>
   ...
   type Response is record
      ...
      --  error might be interpreted by the frontend and is not shown in the
      --  UI. Some predefined values exist.
      a_body : Any_Value;
   ...
   type ExceptionDetails is record
      ...
      --  Optional expression that can be evaluated in the current scope to
      --  obtain the exception object.
   ...
   type DataBreakpointInfoArguments is record
      ...
      --  information for. If variablesReference isn["2019"]t provided,
   ...
   type Source is record
      ...
      --  through the SourceRequest (even if a path is specified). A
      --  sourceReference is only valid for a session, so it must not be
      ...
      --  An optional hint for how to present the source in the UI. A value
      --  of 'deemphasize' can be used to indicate that the source is not
      ...
      --  An optional list of sources that are related to this source. These
      --  may be the source that generated this source.
      adapterData : Any_Value;
      --  Optional data that a debug adapter might want to loop through the
      --  client. The client should leave the data intact and persist it
      --  across sessions. The client should not interpret the data.
   ...
   type Module is record
      ...
      path : VSS.Strings.Virtual_String;
      --  optional but recommended attributes. always try to use these first
      --  before introducing additional attributes.
      --
   ...
   type RestartArguments is new Any_Object with null record;
   ...
   type BreakpointLocationsArguments is record
      ...
      --  Optional start column of range to search possible breakpoint
      --  locations in. If no start column is given, the first column in
      --  the start line is assumed.
      endLine : Optional_Integer;
      --  Optional end line of range to search possible breakpoint locations
      --  in. If no end line is given, then the end line is assumed to be the
      --  start line.
      endColumn : Optional_Integer;
      --  Optional end column of range to search possible breakpoint locations
      --  in. If no end column is given, then it is assumed to be in the last
      --  column of the end line.
   end record;

   type Optional_BreakpointLocationsArguments (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type Capabilities is record
      ...
      --  which requires that both the 'startFrame' and 'levels' arguments
      --  and an optional 'totalFrames' result of the 'StackTrace' request
      --  are supported.
   ...
   type StackTraceResponse_body is record
      ...
      --  The frames of the stackframe. If the array has length zero, there
      --  are no stackframes available. This means that there is no location
      ...
      --  increasing totalFrames values for subsequent requests can be used
   ...
   type Optional_ConfigurationDoneArguments (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type Optional_StepInTargetsResponse_body (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type StackFrame is record
      ...
      --  threads. This id can be used to retrieve the scopes of the frame
      --  with the 'scopesRequest' or to restart the execution of a stackframe.
      name : VSS.Strings.Virtual_String;
      ...
      --  request. Clients should only use this if the debug adapter supports
      --  the 'restart' request (capability 'supportsRestartRequest' is true).
      ...
      --  'label' can be used to indicate that the frame is an artificial frame
      --  that is used as a visual label or separator. A value of 'subtle' can
   ...
   type ExceptionFilterOptions is record
      ...
      --  An optional expression for conditional exceptions. The exception will
      --  break into the debugger if the result of the condition is true.
   end record;
   ...
   type ExceptionBreakpointsFilter is record
      ...
      --  exception filter. This string is typically shown as a hover and
      --  must be translated.
      default : Boolean;
   ...
   type ReadMemoryArguments is record
      ...
      --  Optional offset (in bytes) to be applied to the reference location
      --  before reading data. Can be negative.
      count : Integer;
   ...
   type Variable is record
      ...
      --  hovering over the value. This attribute should only be returned
      --  by a debug adapter if the client has passed the value true for
      ...
      --  optional information to present the children in a paged UI and
      --  fetch them in chunks.
      memoryReference : VSS.Strings.Virtual_String;
      --  Optional memory reference for the variable if the variable represents
      --  executable code, such as a function pointer. This attribute is
      --  only required if the client has passed the value true for the
   ...
   type StoppedEvent_body is record
      ...
      --  string is shown in the UI as is and must be translated.
      threadId : Optional_Integer;
      --  The thread which was stopped.
      preserveFocusHint : Boolean;
      --  A value of true hints to the frontend that this event should not
      --  change the focus.
      ...
      --  multiple breakpoints: - Different types of breakpoints map to
      --  the same location. - Multiple source breakpoints get collapsed to
      --  the same instruction by the compiler/runtime. - Multiple function
      --  breakpoints with different function names map to the same location.
   end record;
   ...
   type CompletionsArguments is record
      ...
      text : VSS.Strings.Virtual_String;
      --  One or more source lines. Typically this is the text a user has typed
      --  into the debug console before he asked for completion.
      column : Integer;
      --  The character position for which to determine the completion
      --  proposals.
   ...
   type Optional_ExceptionInfoResponse_body (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type SetExpressionResponse_body is record
      ...
      a_type : VSS.Strings.Virtual_String;
      --  The optional type of the value. This attribute should only be
      --  returned by a debug adapter if the client has passed the value
      --  true for the 'supportsVariableType' capability of the 'initialize'
      --  request.
      presentationHint : Optional_VariablePresentationHint;
      --  Properties of a value that can be used to determine how to render the
      --  result in the UI.
      variablesReference : Optional_Integer;
      --  If variablesReference is > 0, the value is structured and its
      --  children can be retrieved by passing variablesReference to the
      --  VariablesRequest. The value should be less than or equal to
      --  2147483647 (2^31-1).
      namedVariables : Optional_Integer;
      --  The number of named child variables. The client can use this optional
      ...
      indexedVariables : Optional_Integer;
      --  The number of indexed child variables. The client can use this
      --  optional information to present the variables in a paged UI and fetch
      --  them in chunks. The value should be less than or equal to 2147483647
      --  (2^31-1).
   end record;

   type Optional_SetExpressionResponse_body (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type OutputEvent_body is record
      ...
      --  'variablesReference' to the 'variables' request. The value should
      --  be less than or equal to 2147483647 (2^31-1).
      source : Optional_Source;
      ...
      data : Any_Value;
      --  Optional data to report. For the 'telemetry' category the data will
      --  be sent to telemetry, for the other categories the data is shown in
      --  JSON format.
   end record;
   ...
   type Scope is record
      ...
      --  this optional information to present the variables in a paged UI
   ...
   type DisassembleArguments is record
      ...
      --  Optional offset (in instructions) to be applied after the byte offset
      --  (if any) before disassembling. Can be negative.
   ...
   type DataBreakpointInfoResponse_body is record
      ...
      canPersist : Boolean;
      --  Optional attribute indicating that a potential data breakpoint could
      --  be persisted across sessions.
   end record;

   type Optional_DataBreakpointInfoResponse_body (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type FunctionBreakpoint is record
      ...
      condition : VSS.Strings.Virtual_String;
      --  An optional expression for conditional breakpoints. It
      --  is only honored by a debug adapter if the capability
      --  'supportsConditionalBreakpoints' is true.
      hitCondition : VSS.Strings.Virtual_String;
      --  An optional expression that controls how many hits of the breakpoint
      --  are ignored. The backend is expected to interpret the expression
   ...
   type SetExceptionBreakpointsArguments is record
      ...
      --  Configuration options for selected exceptions. The attribute
      --  is only honored by a debug adapter if the capability
   ...
   type Optional_RunInTerminalResponse_body (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type DisconnectArguments is record
      ...
      --  whatever it thinks is best. The attribute is only honored by a
      --  debug adapter if the capability 'supportTerminateDebuggee' is true.
   end record;
   ...
   type GotoTargetsArguments is record
      ...
      column : Optional_Integer;
      --  An optional column location for which the goto targets are
      --  determined.
   end record;
   ...
   type Optional_SetDataBreakpointsResponse_body (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type Message is record
      ...
      --  An optional url where additional information about this message can
      --  be found.
      urlLabel : VSS.Strings.Virtual_String;
      --  An optional label that is presented to the user as the UI for opening
      --  the url.
   end record;
   ...
   type ContinueResponse_body is record
      ...
      --  continued all threads instead. If this attribute is missing a value
      --  of 'true' is assumed for backward compatibility.
   end record;
   ...
   type EvaluateArguments is record
      ...
      format : Optional_ValueFormat;
      --  Specifies details on how to format the Evaluate result. The
   ...
   type Breakpoint is record
      ...
      --  location).
      message : VSS.Strings.Virtual_String;
      --  An optional message about the state of the breakpoint. This is shown
      --  to the user and can be used to explain why a breakpoint could not be
      --  verified.
      source : Optional_Source;
      ...
      column : Optional_Integer;
      --  An optional start column of the actual range covered by the
      --  breakpoint.
      endLine : Optional_Integer;
      ...
      offset : Optional_Integer;
      --  An optional offset from the instruction reference. This can be
      --  negative.
   end record;
   ...
   type ReadMemoryResponse_body is record
      ...
      --  bytes that must be skipped before a subsequent 'readMemory' request
      --  will succeed.
   ...
   type VariablesArguments is record
      ...
      format : Optional_ValueFormat;
      --  Specifies details on how to format the Variable values. The
      --  attribute is only honored by a debug adapter if the capability
      --  'supportsValueFormattingOptions' is true.
   end record;
   ...
   type DisassembledInstruction is record
      ...
      --  Optional raw bytes representing the instruction and its operands, in
   ...
   type TerminatedEvent_body is record
      ...
      --  to request that the front end restarts the session. The value is
      --  not interpreted by the client and passed unmodified as an attribute
   ...
   type CompletionItem is record
      ...
      start : Optional_Integer;
      --  This value determines the location (in the CompletionsRequest's
      --  'text' attribute) where the completion text is added. If missing the
      --  text is added at the location specified by the CompletionsRequest's
      --  'column' attribute.
      length : Optional_Integer;
      --  This value determines how many characters are overwritten by the
      --  completion text. If missing the value 0 is assumed which results
      --  in the completion text being inserted.
      selectionStart : Optional_Integer;
      --  Determines the start of the new selection after the text has been
      --  inserted (or replaced). The start position must in the range 0 and
      --  length of the completion text. If omitted the selection starts at
      ...
      --  inserted (or replaced). The selection can not extend beyond the
      --  bounds of the completion text. If omitted the length is assumed to
      --  be 0.
   end record;
   ...
   type BreakpointEvent_body is record
      ...
      --  The 'id' attribute is used to find the target breakpoint and the
      --  other attributes are used as the new values.
   end record;
   ...
   type GotoTarget is record
      ...
      instructionPointerReference : VSS.Strings.Virtual_String;
      --  Optional memory reference for the instruction pointer value
   ...
   type BreakpointLocation is record
      ...
      endLine : Optional_Integer;
      --  Optional end line of breakpoint location if the location covers a
      --  range.
      endColumn : Optional_Integer;
      --  Optional end column of breakpoint location if the location covers a
      --  range.
   end record;
   ...
   type EvaluateResponse_body is record
      ...
      a_type : VSS.Strings.Virtual_String;
      --  The optional type of the evaluate result. This attribute should only
      --  be returned by a debug adapter if the client has passed the value
      --  true for the 'supportsVariableType' capability of the 'initialize'
      --  request.
      presentationHint : Optional_VariablePresentationHint;
      ...
      --  and its children can be retrieved by passing variablesReference
      --  to the VariablesRequest. The value should be less than or equal
      --  to 2147483647 (2^31-1).
      namedVariables : Optional_Integer;
      --  The number of named child variables. The client can use this optional
      --  information to present the variables in a paged UI and fetch them
      ...
      --  memory address contained in the pointer. This attribute should be
      --  returned by a debug adapter if the client has passed the value true
      --  for the 'supportsMemoryReferences' capability of the 'initialize'
      --  request.
   end record;
   ...
   type Optional_SetBreakpointsResponse_body (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type InstructionBreakpoint is record
      instructionReference : VSS.Strings.Virtual_String;
      --  The instruction reference of the breakpoint. This should be a memory
      --  or instruction pointer reference from an EvaluateResponse, Variable,
   ...
   type InvalidatedEvent_body is record
      ...
      --  Optional set of logical areas that got invalidated. This property has
      --  a hint characteristic: a client can only be expected to make a 'best
      --  effort' in honouring the areas but there are no guarantees. If this
      --  property is missing, empty, or if values are not understand the
   ...
   type Optional_LoadedSourcesResponse_body (Is_Set : Boolean := False)
   is record
      case Is_Set is
         when True =>
   ...
   type Thread_Variable_Reference (Element : not null access Thread)
   is null record with
   ...
   type Checksum_Variable_Reference (Element : not null access Checksum)
   is null record with
      Implicit_Dereference => Element;
   ...
   type Checksum_Constant_Reference
     (Element : not null access constant Checksum) is null record with
      Implicit_Dereference => Element;
   ...
   type Breakpoint_Variable_Reference (Element : not null access Breakpoint)
   is null record with
      Implicit_Dereference => Element;
   ...
   type Breakpoint_Constant_Reference
     (Element : not null access constant Breakpoint) is null record with
      Implicit_Dereference => Element;
   ...
   type StepInTarget_Variable_Reference
     (Element : not null access StepInTarget) is null record with
      Implicit_Dereference => Element;
   ...
   type StepInTarget_Constant_Reference
     (Element : not null access constant StepInTarget) is null record with
      Implicit_Dereference => Element;
   ...
   type FunctionBreakpoint_Variable_Reference
     (Element : not null access FunctionBreakpoint) is null record with
      Implicit_Dereference => Element;
   ...
   type DataBreakpoint_Variable_Reference
     (Element : not null access DataBreakpoint) is null record with
      Implicit_Dereference => Element;
   ...
   type DataBreakpoint_Constant_Reference
     (Element : not null access constant DataBreakpoint) is null record with
      Implicit_Dereference => Element;
   ...
   type ExceptionOptions_Variable_Reference
     (Element : not null access ExceptionOptions) is null record with
      Implicit_Dereference => Element;
   ...
   type ExceptionOptions_Constant_Reference
     (Element : not null access constant ExceptionOptions) is null record with
      Implicit_Dereference => Element;
   ...
   type GotoTarget_Variable_Reference (Element : not null access GotoTarget)
   is null record with
      Implicit_Dereference => Element;
   ...
   type GotoTarget_Constant_Reference
     (Element : not null access constant GotoTarget) is null record with
      Implicit_Dereference => Element;
   ...
   type InvalidatedAreas_Variable_Reference
     (Element : not null access Enum.InvalidatedAreas) is null record with
      Implicit_Dereference => Element;
   ...
   type BreakpointLocation_Variable_Reference
     (Element : not null access BreakpointLocation) is null record with
      Implicit_Dereference => Element;
   ...
   type InstructionBreakpoint_Variable_Reference
     (Element : not null access InstructionBreakpoint) is null record with
      Implicit_Dereference => Element;
   ...
   type StackFrame_Variable_Reference (Element : not null access StackFrame)
   is null record with
      Implicit_Dereference => Element;
   ...
   type StackFrame_Constant_Reference
     (Element : not null access constant StackFrame) is null record with
      Implicit_Dereference => Element;
   ...
   type Scope_Variable_Reference (Element : not null access Scope)
   is null record with
      Implicit_Dereference => Element;
   ...
   type Scope_Constant_Reference (Element : not null access constant Scope)
   is null record with
      Implicit_Dereference => Element;
   ...
   type Variable_Variable_Reference (Element : not null access Variable)
   is null record with
      Implicit_Dereference => Element;
   ...
   type Variable_Constant_Reference
     (Element : not null access constant Variable) is null record with
      Implicit_Dereference => Element;
   ...
   type Source_Variable_Reference (Element : not null access Source)
   is null record with
      Implicit_Dereference => Element;
   ...
   type SourceBreakpoint_Variable_Reference
     (Element : not null access SourceBreakpoint) is null record with
      Implicit_Dereference => Element;
   ...
   type SourceBreakpoint_Constant_Reference
     (Element : not null access constant SourceBreakpoint) is null record with
      Implicit_Dereference => Element;
   ...
   type ChecksumAlgorithm_Variable_Reference
     (Element : not null access Enum.ChecksumAlgorithm) is null record with
      Implicit_Dereference => Element;
   ...
   type ExceptionBreakpointsFilter_Variable_Reference
     (Element : not null access ExceptionBreakpointsFilter) is null record with
      Implicit_Dereference => Element;
   ...
   type CompletionItem_Variable_Reference
     (Element : not null access CompletionItem) is null record with
      Implicit_Dereference => Element;
   ...
   type CompletionItem_Constant_Reference
     (Element : not null access constant CompletionItem) is null record with
      Implicit_Dereference => Element;
   ...
   type ExceptionPathSegment_Variable_Reference
     (Element : not null access ExceptionPathSegment) is null record with
      Implicit_Dereference => Element;
   ...
   type DisassembledInstruction_Variable_Reference
     (Element : not null access DisassembledInstruction) is null record with
      Implicit_Dereference => Element;
   ...
   type Module_Variable_Reference (Element : not null access Module)
   is null record with
      Implicit_Dereference => Element;
   ...
   type ExceptionFilterOptions_Variable_Reference
     (Element : not null access ExceptionFilterOptions) is null record with
      Implicit_Dereference => Element;
   ...
   type ColumnDescriptor_Variable_Reference
     (Element : not null access ColumnDescriptor) is null record with
      Implicit_Dereference => Element;
   ...
   type ColumnDescriptor_Constant_Reference
     (Element : not null access constant ColumnDescriptor) is null record with
      Implicit_Dereference => Element;
   ...
   type ExceptionDetails_Variable_Reference
     (Element : not null access ExceptionDetails) is null record with
      Implicit_Dereference => Element;
   ...
   type ExceptionDetails_Constant_Reference
     (Element : not null access constant ExceptionDetails) is null record with
      Implicit_Dereference => Element;
...
private
   ...
   type FunctionBreakpoint_Vector is new Ada.Finalization.Controlled with
   record
   ...
   type BreakpointLocation_Vector is new Ada.Finalization.Controlled with
   record
   ...
   type InstructionBreakpoint_Vector is new Ada.Finalization.Controlled with
   record
   ...
   type ExceptionBreakpointsFilter_Vector is new Ada.Finalization
     .Controlled with
   record
   ...
   type ExceptionPathSegment_Vector is new Ada.Finalization.Controlled with
   record
   ...
   type DataBreakpointAccessType_Vector is new Ada.Finalization.Controlled with
   record
   ...
   type DisassembledInstruction_Vector is new Ada.Finalization.Controlled with
   record
   ...
   type ExceptionFilterOptions_Vector is new Ada.Finalization.Controlled with
   record
