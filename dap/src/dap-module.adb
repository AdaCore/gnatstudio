...
package body DAP.Module is
   ...
   package DAP_Client_Vectors is new
     Ada.Containers.Vectors
       (Positive,
        DAP.Clients.DAP_Client_Access,
        "=" => DAP.Clients."=");
   subtype DAP_Client_Vector is DAP_Client_Vectors.Vector;

   procedure Free is new
     Ada.Unchecked_Deallocation
       (DAP.Clients.DAP_Client'Class,
        DAP.Clients.DAP_Client_Access);

   type DAP_Module_Record
     (Kernel : access GPS.Kernel.Kernel_Handle_Record'Class)
   is new Module_ID_Record with record
   ...
   overriding
   procedure Destroy (Id : in out DAP_Module_Record);

   type DAP_Module is access all DAP_Module_Record'Class;

   function Get_Current_Debugger
     (Id : DAP_Module) return DAP.Clients.DAP_Client_Access;

   function Get_Debugger
     (Id : DAP_Module; Num : Client_Id_Type)
      return DAP.Clients.DAP_Client_Access;

   overriding
   function Tooltip_Handler
     (Module : access DAP_Module_Record; Context : Selection_Context)
      return Gtk_Widget;
   ...
   function Filter_Matches_Primitive
     (Filter : access Has_Debuggers_Filter; Context : Selection_Context)
      return Boolean;

   type Debugger_Initialized_Filter is new Action_Filter_Record
   with null record;
   overriding
   function Filter_Matches_Primitive
     (Filter : access Debugger_Initialized_Filter; Context : Selection_Context)
      return Boolean;

   type Debugger_Available_Filter is new Action_Filter_Record with null record;
   overriding
   function Filter_Matches_Primitive
     (Filter : access Debugger_Available_Filter; Context : Selection_Context)
      return Boolean;
   ...
   type Debuggee_Running_Filter is new Action_Filter_Record with null record;
   overriding
   function Filter_Matches_Primitive
     (Filter : access Debuggee_Running_Filter; Context : Selection_Context)
      return Boolean;

   type No_Debugger_Or_Available_Filter is new Action_Filter_Record
   with null record;
   overriding
   function Filter_Matches_Primitive
   ...
   type No_Debugger_Or_Initialized_Filter is new Action_Filter_Record
   with null record;
   overriding
   function Filter_Matches_Primitive
   ...
   overriding
   function Execute
     (Command : access Continue_Command; Context : Interactive_Command_Context)
      return Command_Return_Type;
   --  Debug->Continue menu
   ...
   overriding
   function Execute
     (Command : access Nexti_Command; Context : Interactive_Command_Context)
      return Command_Return_Type;
   ...
   overriding
   function Execute
     (Command : access Step_Command; Context : Interactive_Command_Context)
      return Command_Return_Type;
   --  Debug->Step menu
   ...
   overriding
   function Execute
     (Command : access Stepi_Command; Context : Interactive_Command_Context)
      return Command_Return_Type;
   ...
   begin
      ...
      if DAP_Module_ID.Clients.Is_Empty then
         ...
         GPS.Kernel.MDI.Load_Perspective (DAP_Module_ID.Get_Kernel, "Debug");
   ...
   overriding
   procedure Destroy (Id : in out DAP_Module_Record) is
   ...
   is
      ...
      Client : constant DAP.Clients.DAP_Client_Access := Get_Current_Debugger;
      W      : Gtk_Widget;
      Label  : Gtk_Label;
   begin
      if Module = null or else Client = null or else not Client.Is_Stopped then
         return null;
      end if;
      ...
      declare
         Variable_Name : constant String :=
           DAP.Contexts.Get_Variable_Name (Context, Dereference => False);
      begin
         if Variable_Name = ""
           or else
         ...
         DAP.Clients.Evaluate.Send_Get_Value_Of_Request
           (Client => Client.all, Label => Label, Entity => Variable_Name);
         ...
         Label.Modify_Font (GPS.Kernel.Preferences.View_Fixed_Font.Get_Pref);
   ...
   begin
      ...
      if DAP_Module_ID /= null and then not DAP_Module_ID.Clients.Is_Empty then
      ...
      DAP.Module.Breakpoints.Save_Persistent_Breakpoints (Kernel);
   ...
   is
      ...
      Show_Project_In_Menu : constant Boolean :=
        GPS.Kernel.Modules.UI.Group_Mains_Into_Projects (Kernel, Mains.Length);
   ...
   begin
      ...
      if Pref = Preference (DAP.Modules.Preferences.Continue_To_Line_Buttons)
      then
         declare
         ...
         begin
            if Client /= null then
               if DAP.Modules.Preferences.Continue_To_Line_Buttons.Get_Pref
               then
                  Enable_Continue_To_Line_On_Editors (DAP_Module_ID.Kernel);
               else
   ...
   function Execute
     (Command : access Terminate_Command;
      Context : Interactive_Command_Context) return Command_Return_Type is
   begin
      if DAP_Module_ID /= null and then DAP_Module_ID.Current_Debuger_ID /= 0
      then
   ...
   begin
      if Client = null then
         ...
         Start_Executable
           (Kernel => Kernel, Client => Client, Display_Start_Dialog => True);
   ...
   is
      ...
      begin
         ...
         Args_Combo :=
           Dialog.Add_Combo
             (Message => "Run arguments:", Key => Run_Arguments_History_Key);
         ...
         declare
            Stop : constant Boolean := Stop_At_Beginning_Button.Get_Active;
            Args : constant String :=
              String_Utils.Strip_Ending_Linebreaks (Args_Combo.Get_Text);
         begin
            Dialog.Destroy;
            if Response = Gtk_Response_OK then
               Client.Launch_Executable
                 (Executable        => Client.Get_Executable,
                  Executable_Args   =>
                    VSS.Strings.Conversions.To_Virtual_String (Args).Split
         ...
         begin
            ...
            if PID /= "" then
               DAP.Clients.Attach.Send_Attach_Request
                 (Client => Client.all, PID => Integer'Value (PID));
            end if;
         end;
   ...
   begin
      if Client /= null then
         ...
         DAP.Clients.Disconnect.Send_Disconnect_Request
           (Client => Client.all, Terminate_Debuggee => False);
   ...
   begin
      ...
      begin
         ...
         if Selected_File /= No_File then
            ...
            Client.Launch_Executable
              (Executable => Selected_File, Stop_At_Beginning => True);
         end if;
   ...
   function Filter_Matches_Primitive
     (Filter : access Has_Debuggers_Filter; Context : Selection_Context)
      return Boolean is
   begin
      return DAP_Module_ID /= null and then not DAP_Module_ID.Clients.Is_Empty;
   ...
   begin
      return
        DAP.Module.Get_Current_Debugger /= null
   ...
   begin
      return
        Client /= null and then Client.Get_Debuggee_Start_Method = Attached;
   end Filter_Matches_Primitive;
   ...
   function Get_Debugger
     (Id : Client_Id_Type) return DAP.Clients.DAP_Client_Access is
   begin
   ...
   function Get_Current_Debugger
     (Id : DAP_Module) return DAP.Clients.DAP_Client_Access is
   begin
      if Id = null then
         return null;
      end if;
   ...
   begin
      if DAP_Module_ID = null or else DAP_Module_ID.Clients.Is_Empty then
         return;
      end if;
      ...
      if DAP_Module_ID.Clients.Is_Empty then
         ...
         begin
            ...
            DAP.Module.Breakpoints.On_Debugging_Terminated (Kernel);
   ...
   procedure Set_Current_Debugger (Current : DAP.Clients.DAP_Client_Access) is
      ...
      begin
         if Id = null
           or else
             (Current /= null and then Id.Current_Debuger_ID = Current.Id)
         then
            return False;
         end if;
   ...
   begin
      if Set_Current_Debugger (DAP_Module_ID, Current) and then Current /= null
      then
