...
package DAP.Requests.SetVariable is
   ...
   type Set_Variable_DAP_Request is abstract new DAP_Request with record
      Parameters : aliased DAP.Tools.SetVariableRequest :=
        DAP.Tools.SetVariableRequest'(seq => 0, arguments => <>);
   end record;
   ...
   overriding
   procedure Set_Seq (Self : in out Set_Variable_DAP_Request; Id : Integer);

   overriding
   function Method (Self : in out Set_Variable_DAP_Request) return String
   is ("setVariable");

end DAP.Requests.SetVariable;
