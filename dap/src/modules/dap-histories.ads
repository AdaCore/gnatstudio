...
package DAP.Histories is
   ...
   function Get_Current (History : History_List_Access) return Data_Type;
   --  Return the item currently pointed to.
   --  No_Such_Item is raised if the list is empty.

   procedure Set_Current (History : History_List_Access; Data : Data_Type);
   --  Change the current item data.

   procedure Move_To_Previous (History : History_List_Access);
   ...
   procedure Move_To_Next (History : History_List_Access);
   ...
   function Get_Current_Repeat_Num
     (History : History_List_Access) return Natural;
   ...
   procedure Free (History : History_List_Access);
...
private
   ...
   type Hlist_Access is access all Hlist;

   package Hlist_Container is new
     Ada.Containers.Vectors (Positive, Hlist_Access);

   use Hlist_Container;
   use type Ada.Containers.Count_Type;

   type History_List is record
      Data     : Hlist_Container.Vector;
