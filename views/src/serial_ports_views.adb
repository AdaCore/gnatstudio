...
package body Serial_Ports_Views is
   ...
   overriding
   procedure On_Destroy (View : not null access Serial_Ports_View_Record);
   ...
   function Interpret_Command_Handler
     (Console      : access Interactive_Console_Record'Class;
      Input        : String;
      View_Address : System.Address) return String;
   --  Send intput into port and return the output if any exists.

   package Serial_Ports_View is new
   ...
   package Strings_Sets is new Ada.Containers.Indefinite_Ordered_Sets (String);

   package Timeout_Kernel is new Glib.Main.Generic_Sources (Kernel_Handle);

   function Convert is new
     Ada.Unchecked_Conversion (System.Address, Serial_Port_View);
   ...
   begin
      ...
      Set_Font_And_Colors (View.Console.Get_View, Fixed_Font => True);
   ...
   overriding
   procedure On_Destroy (View : not null access Serial_Ports_View_Record) is
   begin
   ...
   begin
      ...
      Widget_Callback.Object_Connect
        (View.Ports_Refresh,
         Gtk.Tool_Button.Signal_Clicked,
         On_Ports_Refresh_Clicked'Access,
         View);
   ...
   procedure On_Port_Switched (Self : access Gtk_Widget_Record'Class) is
   ...
   procedure On_Status_Clicked (Self : access Gtk_Widget_Record'Class) is
      View : constant Serial_Port_View := Serial_Port_View (Self);
   begin
         ...
         begin
            Get_Property
              (Property,
               ...
               Name  => Port_Property_Name,
               Found => Found);
            if Found and then Ports.Contains (Property.Value.all) then
               Add_Ports;
   ...
   begin
      ...
      Set
        (Current_Port,
         Data_Rate'Value (View.Rates.Get_Selected_Item),
         Block   => False,
         Timeout => 0.01);
      ...
      View.Is_Active := True;
      View.Timeout :=
        Timeout_Kernel.Timeout_Add (250, Idle_Read'Access, View.Kernel);
   ...
   is
      ...
      function As_Pointer is new
        Ada.Unchecked_Conversion (System.Address, Constrained_SEA_Access);
   ...
   is
      ...
      type Constrained_String_Access is access all String (1 .. Size);

      function As_Pointer is new
        Ada.Unchecked_Conversion (System.Address, Constrained_String_Access);
   ...
   function Idle_Read (Kernel : Kernel_Handle) return Boolean is
      View : constant Serial_Port_View :=
        Serial_Port_View (Serial_Ports_View.Retrieve_View (Kernel, True));
   begin
      if View = null or else View.In_Destruction or else not View.Is_Active
      then
         return False;
      end if;
