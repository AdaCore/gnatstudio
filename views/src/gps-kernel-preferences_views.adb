...
package body GPS.Kernel.Preferences_Views is
   ...
   type Overall_Preferences_Search_Provider is new Overall_Search_Provider
   with null record;
   ...
   type GPS_Preferences_Editor_Record is
     new Generic_Views.View_Record
     and Preferences_Editor_Interface
   with record
   ...
   function Get_Page_View
     (Self : not null access GPS_Preferences_Editor_Record; Page_Name : String)
      return Gtk.Widget.Gtk_Widget;
   overriding
   function Is_Displaying_Hidden_Preferences
     (Self : not null access GPS_Preferences_Editor_Record) return Boolean
   is (Self.Pages_Notebook.Get_Current_Page = Self.Hidden_Page_Index);
   ...
   package Preferences_Editor_Views is new
   ...
   begin
      return
        new Custom_Preferences_Search_Result'
          (Kernel   => Self.Kernel,
           Provider => Self,
           Score    => Score,
           Short    => Short,
   ...
   begin
      return
        new Custom_Plugins_Search_Result'
          (Kernel      => Self.Kernel,
           Provider    => Self,
           Score       => Score,
           Short       => Short,
   ...
   begin
      ...
      Editor.Display_Pref (Pref => Self.Pref, Highlight => False);
   end Execute;
   ...
   begin
      ...
      Editor.Display_Pref (Pref => Self.Pref, Highlight => True);
   ...
   begin
      ...
      Append_Menu (Menu => Menu, Kernel => View.Kernel, Pref => Show_Advanced);
   ...
   is
      ...
      Page_Name_In_Model : constant String :=
        (if Page_Name = "" then Advanced_Page_Name else Page_Name);
   begin
      Self.Highlighted_Pref := null;

      --  Try to get the page containing it
      Page_Found := Find_Page_Iter (Self, Page_Iter, Page_Name_In_Model);
   ...
   begin
      if Self.Highlighted_Pref /= null then
         ...
         Page_View.Set_Pref_Highlighted
           (Pref => Self.Highlighted_Pref, Highlight => False);
      end if;
      ...
      if Highlight then
         Page_View := Get_Selected_Page_View (Self);
         Page_View.Set_Pref_Highlighted (Pref => Pref, Highlight => Highlight);
   ...
   is
      ...
      Page_Iter  : Gtk_Tree_Iter;
   begin
      Page_Found := Find_Page_Iter (Self, Page_Iter, Page_Name);
   ...
   begin
      return
        Preferences_Page_View
          (Self.Pages_Notebook.Get_Nth_Page (Current_Page_Index));
         ...
         if Page_Index = -1 then
            declare
               ...
               Page_Name   : constant String :=
                 (if M_Page_Name = Advanced_Page_Name
                  then ""
                  else M_Page_Name);
            ...
            begin
               ...
               Pref_View.Model.Set (Child_Iter, Page_Index_Column, Page_Index);
            ...
            declare
               P : constant Custom_Preferences_Search_Provider_Access :=
                 Custom_Preferences_Search_Provider_Access
                   (Editor.Provider_Registry.Get (Provider_Preferences));
            begin
   ...
   begin
      while First <= Page_Name'Last loop
         ...
         while Last <= Page_Name'Last and then Page_Name (Last) /= '/' loop
            Last := Last + 1;
         end loop;
   ...
   is
      Manager             : constant Preferences_Manager :=
        Self.Kernel.Preferences;
      Filename            : constant Virtual_File :=
        Self.Kernel.Preferences_File;
      Main_Pane           : Gtk_Paned;
   ...
   begin
      ...
      Self.Pack_Start (Child => Main_Pane, Expand => True, Fill => True);
      ...
      Main_Pane.Pack1
        (Child => Scrolled_Pages_Tree, Resize => False, Shrink => False);
      ...
      Main_Pane.Pack2
        (Child => Self.Pages_Notebook, Resize => True, Shrink => False);
   ...
   procedure On_Apply_Button_Clicked (Widget : access GObject_Record'Class) is
      Editor : constant GPS_Preferences_Editor :=
        GPS_Preferences_Editor (Widget);
   begin
      ...
      Show_Advanced :=
        Kernel.Get_Preferences.Create_Invisible_Pref
          (Name    => "show-advanced-preferences",
           ...
           Doc     =>
             ("Show the advanced preferences in the preferences editor"
              & " dialog."));
