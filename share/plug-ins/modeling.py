...
class GMC_Module(modules.Module):
    ...
    def __handle_MDL2JSON_compilation_event(self, process, status, output):
        ...
        # Otherwise the compilation failed, issue an error dialog
    ...
    def __model_file(self):
        ...
        return GPS.Project.root().get_attribute_as_string(
            package="GMC", attribute=SOURCE_MODEL_ATTR)
    ...
    def __output_directory(self):
        ...
        return GPS.Project.root().get_attribute_as_string(
            package="GMC", attribute=OUTPUT_DIRECTORY_ATTR)
    ...
    def __setup_menus(self):
        ...
        gps_utils.make_interactive(
            callback=self.__compile_model, name=COMPILE_MODEL_ACTION)
    ...
    def __visualize_model(
        ...
        if self.__present(model_file) and model_file.language() == "simulink":
            print "file " + model_file.name() + " being visualized"

            # Compile a Simulink model into JSON only when the MDL2JSON tool is
            # available.

            if MDL2JSON_EXEC:
                print "MDL2JSON present"
                ...
                switches = re.sub(
                    pattern=OPTION_FLATTEN, repl="", string=switches)
            ...
            # Otherwise MDL2JSON is not available, issue an error dialog

            else:
                return False

        # Otherwise the file being opened is not a Simulink model
...
class Language:
    ...
    def __init__(self, name, comment_start, comment_end):
        ...
        """
        self.name = name
        self.header = comment_start + GMC_HEADER + comment_end
        self.block_start = comment_start + BLOCK_START + comment_end
        self.block_end = comment_start + BLOCK_END + comment_end
