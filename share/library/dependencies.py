...
and reports dependencies that are in fact not needed (and since they will
impact what's put on the linker command line, you should remove them),
as well as indirect dependencies (A depends on B depends on C, therefore
A should depend on C directly), which it is generally cleaner to put
...
The output of this script can be viewer either as textual output in the
