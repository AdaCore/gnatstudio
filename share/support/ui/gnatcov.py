...
# Names for each for the source instrumentation based build targets
SETUP_TARGET = "GNATcov Setup Instrumentation Runtime"
INSTRUMENT_TARGET = "Run GNATcov with instrumentation"
BUILD_INSTR_TARGET = "GNATcov Build Instrumented Main"
COVERAGE_INSTR_TRAGET = "Generate GNATcov Instrumented Main Report"

INSTRUMENT_HINT = (
    f"Ensure the coverage runtime is setup, use the '{SOURCE_TRACES_MENU}/Setup/"
    f"Setup coverage runtime' menu to do so, or use the '{SOURCE_TRACES_MENU}"
    "/Select prebuilt runtime' menu to register a prebuilt runtime within GNAT studio."
)
COVERAGE_HINT = (
    "Ensure the trace generated by the executable is named [executable_name].srctrace"
    " and located in the object directory of the project, or adapt the default command"
    " line of this build target. This should be the case if the project is instrumented"
    f" using the '{SOURCE_TRACES_MENU}/Instrumentation/{INSTRUMENT_TARGET}' menu."
)
...
class GNATcovPlugin(Module):
    ...
    SOURCE_TRACES_BUILD_TARGETS = [
        X(
            ...
            X("in-toolbar").children("FALSE"),
            X("in-menu").children("TRUE"),
            X("command-help").children(
                "Set up the coverage runtime."
                "\n\n It is by default set up in a temporary directory,"
                " according to the current target and Ada runtime."
                "\n\n{help_setup}"
            ),
            X("read-only").children("TRUE"),
            X("output-parsers").children(
        ...
        X(
            ...
            X("command-help").children(
                ...
                "\n\n This step assumes the coverage runtime has been"
                " previously setup through GNATstudio, or selected through the"
                f" '{SOURCE_TRACES_MENU}/Setup/Select prebuilt runtime'"
                " menu."
                "\n\n By default, the instrumented sources will produce"
                " coverage traces with a simple filename prefix"
                " ([exe_name].srctrace)."
                "\n\n{help_instrument}"
            ),
            X("read-only").children("TRUE"),
        ...
        X(
            ...
            X("command-help").children(
                ...
                "\n\n This step assumes the sources have been"
                " previously instrumented through GNATstudio, via the"
                f" '{SOURCE_TRACES_MENU}/Run GNATcov with Instrumentation' menu."
            ),
            X("read-only").children("TRUE"),
            X("output-parsers").children(
                "output_chopper utf_converter console_writer end_of_build"
        ...
        X(
            ...
            model="gnatcov-coverage",
            category="_GNATcov_",
            name=COVERAGE_INSTR_TRAGET,
            menu=SOURCE_TRACES_MENU + "/Generate Report/",
        ).children(
            X("target-type").children("executable"),
            X("read-only").children("TRUE"),
            X("command-help").children(
                "Create a coverage report from source traces"
                "\n\n The default switches for this step assume that a single"
                " trace was created for the current executable. It should be"
                " located in the object directory and named"
                " [exe_name].srctrace."
                "\n\nBy default this will produce a report in the expected format"
                " and expected location in order to display the report in the"
                " coverage results view of GNAT studio (xcov+ format, in the"
    ...
    def update_worflow_build_targets(self):
        ...
        if gnatcov_available and not self.__run_gnatcov_wf_build_target:
            ...
            if instrumentation_supported:
                ...
                help_messages = {}
                for command in ["setup", "instrument", "coverage"]:
                    help_messages[f"help_{command}"] = xml.sax.saxutils.escape(
                        GNATcovPlugin.command_help(command)
                    )
                ...
                GPS.parse_xml(
                    list_to_xml(self.SOURCE_TRACES_BUILD_TARGETS[:5]).format(
                        **help_messages
                    )
                )
    ...
    def command_help(command_name):
        """
        Return the help text associated with command_name, removing any lines
        that start with a gnatcoll trace header (i.e. starting with
        "[GNATCOV.")
        """
        lines = (
            GPS.Process(["gnatcov", command_name, "--help"]).get_result().splitlines()
        )
        return "\n".join([line for line in lines if not line.startswith("[GNATCOV.")])
    ...
    def on_compilation_finished(
        self, hook, category, target_name="", mode_name="", status="", cmd="", *args
    ):
        """Called whenever a compilation ends."""

        # If compilation failed, check wether we can find an error message
        # indicating that one of the plugin assumptions is not respected, and
        # emit a suggestion to remediate it.
        if status:
            messages = GPS.Console("Messages").get_text().splitlines()

            # Main assumption concerning the "instrument" command: it relies on
            # the coverage runtime being setup by GNATstudio, or registered via
            # the "select prebuilt runtime" menu.
            if target_name == INSTRUMENT_TARGET:
                # If the command line was actually modified and the coverage
                # runtime is not explicitly passed on the command line, we
                # can't really help the user.
            ...
            # coverage runtime is in the expected location, but this is already
            # checked by the previous build target.

            # No specific assumption for the run build target, if the execution
            # failed it is purely related to user aspects.

            # Main assumption for the "coverage" command: the trace was
