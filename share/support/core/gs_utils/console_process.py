...
class Console_Process(GPS.Console, GPS.Process):
    ...
    def __init__(
        ...
        self.__clear_action.button(toolbar=toolbar_name, label="Clear")
...
class ANSI_Console_Process(Console_Process):
    ...
    def __init__(self, command):
        Console_Process.__init__(
            self, command, force=True, ansi=True, manage_prompt=False
        )
    ...
    def on_key(self, keycode, key, modifier):
        if modifier == 0 or modifier == Shift_Mask:
            ...
            GPS.Logger("CONSOLE").log(
                "keycode=%s key=%s modifier=%s" % (keycode, key, modifier)
            )
