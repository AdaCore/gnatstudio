...
# Disable Libclang-based diagnostics we are using the LSP
# for C/C++.
if not GPS.Logger("GPS.LSP.CPP_SUPPORT").active:
    ...
    class Clang(object):
        ...
        def add_diagnostics(self, ed_buffer):
            ...
            for d in tu.diagnostics:
                # Skip diagnostics that are not in the current file. If
                # diagnostic has no file, it might be a config error (bad
                # switch for example) so we want to show it
                # ??? Did we mean 'name()' (or path) below, instead of just
                # 'name'
                ...
                m = GPS.Message(
                    ...
                    show_in_locations=(
                        show_diags_pref.get() == EDITOR_LOCATIONS),
...
#######################
    ...
    class Clang_Module(Module):
        ...
        def refresh_current_editor(self):
            # Get the current editor but don't open a new editor if none is
            # open
        ...
        def semantic_tree_updated(self, f):
            if self.is_on():
                # The file might have been opened in a QGen browser for
                # instance
    ...
    # We want to remove the del methods on TranslationUnit and Index, because
    # in GS, GS has ownership of those
