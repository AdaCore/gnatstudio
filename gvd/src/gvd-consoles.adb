...
package body GVD.Consoles is
   ...
   type Console_Process_View_Record is abstract new Process_View_Record
   with record
      Console : Interactive_Console;
   end record;

   type Debugger_Console_Record is new Console_Process_View_Record
   with null record;
   ...
   type Debuggee_Console_Record is new Console_Process_View_Record with record
   ...
   package Debugger_MDI_Views is new
   ...
   function Complete_Command
     (Input   : String;
      ...
      Console : System.Address) return String_List_Utils.String_List.Vector;
   ...
   begin
      return
        Debugger_Console (Visual_Debugger_Record (Process.all).Debugger_Text);
   ...
   begin
      if Console.Get_Process /= null then
         ...
         if Match /= Expect_Timeout then
            ...
            Find_MDI_Child
              (Get_MDI (Visual_Debugger (Console.Get_Process).Kernel), Console)
              .Highlight_Child;
         end if;
      end if;
   ...
   exception
      when Process_Died =>
         ...
         Console.Close_TTY;
         if Visual_Debugger (Console.Get_Process).Debuggee_TTY /= Null_TTY then
   ...
   begin
      ...
      elsif not Get_Process (Visual_Debugger (C.Get_Process).Debugger)
                  .Command_In_Process
      then
   ...
   begin
      ...
      then
         if Visual_Debugger (Console.Get_Process).Debuggee_TTY = Null_TTY then
            Allocate_TTY (Visual_Debugger (Console.Get_Process).Debuggee_TTY);
            Created := True;
         end if;
         ...
         then
            Set_TTY
              (Visual_Debugger (Console.Get_Process).Debugger,
               TTY_Name (Visual_Debugger (Console.Get_Process).Debuggee_TTY));
         end if;
      end if;
   ...
   function Debugger_Console_Has_Focus
     (Process : not null access Visual_Debugger_Record'Class) return Boolean is
   begin
   ...
   procedure Display_In_Debugger_Console
     (Process        :
        not null access GVD.Process.Visual_Debugger_Record'Class;
      Text           : String;
   ...
   begin
      if Process /= null and then Process.Debuggee_Console = null then
   ...
   begin
      ...
      if GNAT.TTY.TTY_Supported or else Active (Menu_Generation_Handle) then
         Register_Action
           (Kernel,
