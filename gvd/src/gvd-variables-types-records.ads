...
package GVD.Variables.Types.Records is
   ...
   function Find_Variant_Part
     (Self     : not null access GVD_Record_Type;
      Field    : Positive;
      Contains : String) return GVD_Type_Holder;
   ...
   procedure Draw_Border
     (Self : not null access GVD_Record_Type; Draw : Boolean := True);
...
private
   ...
   type GVD_Record_Type (Num_Fields : Natural) is new GVD_Generic_Type
   with record
   ...
   overriding
   procedure Clear (Self : not null access GVD_Record_Type);
   ...
   overriding
   procedure Free (Self : not null access GVD_Record_Type);
   ...
   type GVD_Union_Type (Num_Fields : Natural) is
     new GVD_Record_Type (Num_Fields)
   with null record;

   overriding
   function Get_Type_Descr
     (Self : not null access GVD_Union_Type) return String
   is ("Union");
