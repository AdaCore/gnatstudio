...
--
--      procedure My_Filter (Pid : Pipes_Id; Str : String) is
--      begin
--         Put_Line (Str);
--      end;
--
...
--  by encapsulating the type Pipes_Id in a protected record. There should not
--  be concurrent calls to Expect.
...
package GNAT.Expect is
   ...
   type Pipes_Id is private;
   --  This covers the standard notion of file descriptor on Unix systems,
   --  and can be associated with pipes, files, ...

   type Pipes_Id_Access is access Pipes_Id;
   ...
   procedure Close (Pid : in out Pipes_Id);
   ...
   function Get_Input_Fd (Pid : Pipes_Id) return GNAT.OS_Lib.File_Descriptor;
   --  Return the input file descriptor associated with Pid.

   function Get_Output_Fd (Pid : Pipes_Id) return GNAT.OS_Lib.File_Descriptor;
   --  Return the output file descriptor associated with Pid.

   function Get_Error_Fd (Pid : Pipes_Id) return GNAT.OS_Lib.File_Descriptor;
   --  Return the error output file descriptor associated with Pid.
   ...
   type Filter_Function is access procedure (Pid : Pipes_Id; Str : String);
   ...
   --  Pid, the filter is called with these new characters in argument.
   --  This filter will be run before all the existing filters, unless
   --  After is True.
   --  Note that input is only generated by called to Send below.
   ...
   procedure Trace_Filter (Pid : Pipes_Id; Str : String);
   ...
   function Expect_Out (Pid : Pipes_Id) return String;
   ...
   function Expect_Out_Match (Pid : Pipes_Id) return String;
   --  Return the string matched by the last Expect call.
   ...
   --  Raised by most subprograms above when the parameter Pid is not a valid
   --  process or is a closed process.

   Process_Died : exception;
   --  Raised by all the expect subprograms if Pid was originally a valid
