...
package body Browsers.Call_Graph is
   ...
   function Initialize
     (View : access Call_Graph_Browser_Record'Class) return Gtk_Widget;
   --  Initialize the browser, and return the focus widget

   package Callgraph_Views is new
   ...
   overriding
   function Name (Self : access Entity_Calls_All_Command) return String
   ...
   type Xref_Text_Record is new Text_Item_Record and Clickable_Item with record
      File : Virtual_File;
      Line : Integer;
   end record;
   ...
   function Save_To_XML
     (Self : not null access Entity_Item_Record) return XML_Utils.Node_Ptr;
   overriding
   ...
   function Initialize
     (View : access Call_Graph_Browser_Record'Class) return Gtk_Widget is
   begin
   ...
   begin
      Editor.Current_View.Cursor_Goto
        (Editor.New_Location_At_Line
           (Basic_Types.Editable_Line_Type (Self.Line)),
         Raise_View => True);
   end On_Click;
   ...
   begin
      if not Is_Predefined_Entity (Item.Entity.Element) then
         ...
         Set_File_Information
           (Context, Files => (1 => Loc.File), Line => Loc.Line);
      end if;
   ...
   begin
      if Node_Entity /= No_Root_Entity and then Is_Subprogram (Node_Entity)
      then
   ...
   begin
         ...
         Str : constant String :=
           Display_Text_Input_Dialog
             (Kernel  => Kernel,
              ...
              Message =>
                -("Computing complete call graph may take a "
                  & "long time."
                  & ASCII.LF
      ...
      begin
         ...
         then
            --  ??? Should check that Decl.Kind is a subprogram
            Examine_Entity_Call_Graph
              (Kernel, Node_Entity, Recursive => True, Max_Count => Max_Items);
         else
            Insert
              (Get_Kernel (Context.Context),
               -"No call graph available for "
   ...
   begin
      if Info /= No_Root_Entity and then Is_Subprogram (Info) then
   ...
   begin
      ...
      Register_Action
        (Kernel,
         "Browser: entity calls (recursive)",
