...
package body Browsers.Call_Graph is
   ...
   function Find_Entity
     (Self   : access Call_Graph_Browser_Record'Class;
   ...
   begin
      Self := new Entity_Item_Record;
      Self.Browser := General_Browser (Browser);
      Self.Entity.Replace_Element (Entity);

      Self.Initialize_Rect (Style => S.Item, Radius => 5.0);
      Setup_Titlebar
        (Self, Browser,
         Name  => Name,
      ...
      Self.Add_Child (Text, Margin => (2.0, 2.0, 2.0, 2.0));
   end Gtk_New;
   ...
   is
      ...
      begin
         if Item.all in Entity_Item_Record'Class
           and then Entity_Item (Item).Entity.Element = Entity
         then
            Result := Entity_Item (Item);
         end if;
      end On_Item;
   ...
   begin
      ...
      Data := new Examine_Ancestors_Data'
        (Commands_User_Data_Record with
           ...
           Max_Count      => Max_Count,
           Parent         => null,
           Count          => (if Newly_Added then 1 else 0));
      Examine_Entity_Call_Graph
        (Entity            => Entity,
         User_Data         => Data,
   ...
   begin
      if Data.Parent /= null then
         Data.Parent.Count := Data.Count;
      else
         if Data.Count /= 0 then
            Data.Browser.Refresh_Layout (Rescale => False);
   ...
   procedure Add_Link_If_Not_Present
     (Browser             : Call_Graph_Browser;
      ...
      Through_Dispatching : Boolean)
   is
      pragma Unreferenced (Through_Dispatching, Ref);
   ...
   begin
      ...
      if Data.Link_From_Item then
         Add_Link_If_Not_Present
           (Data.Browser, Data.Item, It,
            Ref, Is_Renaming, Through_Dispatching);
      else
         Add_Link_If_Not_Present
           (Data.Browser, It, Data.Item,
            Ref, Is_Renaming, Through_Dispatching);
      end if;
