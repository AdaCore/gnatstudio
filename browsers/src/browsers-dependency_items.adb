...
package body Browsers.Dependency_Items is
   ...
   function Initialize
     (View : access Dependency_Browser_Record'Class) return Gtk_Widget;
   --  Creates the dependency browser and returns the focus widget

   package Dependency_Views is new
   ...
   type Xref_Dependency_Browser_Provider_Type is
   ...
   function Save_To_XML
     (Self : not null access File_Item_Record) return XML_Utils.Node_Ptr;
   overriding
   ...
   begin
      Set_File_Information
        (Context, Files => (1 => Item.Source), Project => Project_Of (Item));
   end Set_Context;
   ...
   function Initialize
     (View : access Dependency_Browser_Record'Class) return Gtk_Widget is
   begin
      Browsers.Canvas.Initialize (View);
      Setup_Contextual_Menu
        (Kernel => View.Kernel, Event_On_Widget => View, Context_Func => null);
      ...
      while Iter.Has_Element loop
         Dependencies.Append
           (Dependency_Description_Type'
              (File         => Iter.Element,
               Project_Path =>
                 Iter.Project (Get_Registry (Kernel).Tree.all).Project_Path));
         Iter.Next;
      end loop;
   ...
   begin
      Dependency_Views.Get_Or_Create_View (Kernel => Kernel, Focus => True)
   ...
   is
      ...
      Browser     : constant Dependency_Browser :=
        Dependency_Views.Retrieve_View (Kernel);
      Focus_Child : constant MDI_Child := Get_MDI (Kernel).Get_Focus_Child;
   begin
      ...
      if Browser /= null
        and then
          MDI_Child (Dependency_Views.Child_From_View (Browser)) = Focus_Child
      then
         declare
            Model          : constant Canvas_Model := Browser.Get_View.Model;
   ...
   begin
      ...
      if Browser /= null
        ...
        and then
          Integer
            (Item_Sets.Length (Browser.Get_View.Model.Get_Selected_Items))
          > 1
      then
   ...
   begin
      ...
      if Command = "uses" then
         ...
         declare
            ...
            Iter           : File_Iterator :=
              Kernel.Databases.Find_Dependencies (File, Project);
         begin
   ...
   begin
      ...
      Show_System_Files :=
        Kernel.Get_Preferences.Create_Invisible_Pref
          ("browser_show_system_files", False, Label => -"Show system files");
      Show_Implicit :=
        Kernel.Get_Preferences.Create_Invisible_Pref
          ("browser_show_implicit",
           False,
           Label => -"Show implicit dependencies");

      Register_Action
        (Kernel,
         "Browser: show dependencies for file",
         Command     =>
           new Show_Dependencies_Command'
      ...
      Register_Action
        (Kernel,
         "Browser: show files depending on file",
         Command     =>
           new Show_Dependencies_Command'
   ...
   procedure Add_Link
     (Self     : not null access Dependency_Browser_Record'Class;
      ...
      Explicit : Boolean)
   is
      Link : constant Dependency_Link :=
        Self.Add_Link (From => From, To => To, Explicit => Explicit)
      with Unreferenced;
   begin
      null;
   end Add_Link;
