...
package VCS2.Engines is
   ...
   function Create_Engine
     (Self : not null access VCS_Engine_Factory; Working_Dir : Virtual_File)
   ...
   function Find_Working_Directory
     (Self : not null access VCS_Engine_Factory; File : Virtual_File)
      return Virtual_File
   is abstract;
   ...
   overriding
   procedure Free (Self : in out VCS_Engine);
   ...
   function Guess_VCS_For_Directory
     (Self : not null access VCS_System; Directory : Virtual_File)
      return not null Abstract_VCS_Engine_Access;
   overriding
   procedure Invalidate_All_Caches (Self : not null access VCS_System);
   ...
   procedure On_Start (Self : not null access Task_Visitor) is null;
   ...
   procedure On_Terminate
     (Self : not null access Task_Visitor; VCS : access VCS_Engine'Class)
   is null;
   ...
   procedure On_Tooltip (Self : not null access Task_Visitor; Text : String)
   is null;
   ...
   procedure Async_Fetch_Status_For_Files
     (Self : not null access VCS_Engine; Files : File_Array)
   is null;
   procedure Async_Fetch_Status_For_Project
     (Self : not null access VCS_Engine; Project : Project_Type)
   is null;
   procedure Async_Fetch_Status_For_All_Files
     (Self : not null access VCS_Engine; From_User : Boolean)
   is null;
   ...
   function Default_File_Status
     (Self : not null access VCS_Engine) return VCS_File_Status
   is (Status_No_VCS);
   ...
   procedure Async_Discard_Local_Changes
     (Self : not null access VCS_Engine; Files : GNATCOLL.VFS.File_Array)
   is null;
   ...
   procedure Set_Working_Directory
     (Self : not null access VCS_Engine'Class; Working_Dir : Virtual_File);
   function Working_Directory
     (Self : not null access VCS_Engine'Class) return Virtual_File;
   --  Return the root directory of the working directory.

   function Kernel
     (Self : not null access VCS_Engine'Class) return not null Kernel_Handle;
   --  Return the kernel.

   procedure Set_Run_In_Background
     (Self : not null access VCS_Engine'Class; Background : Boolean);
   ...
   function Get_VCS_Selector
     (Self : not null access VCS_System) return Gtk_Widget;
...
private
   ...
   package VCS_Status_Displays is new
     Ada.Containers.Hashed_Maps
   ...
   package Command_Queues is new
     Ada.Containers.Vectors (Positive, VCS_Command_Access);
