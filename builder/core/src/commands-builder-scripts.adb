...
package body Commands.Builder.Scripts is
   ...
   function Get_Target_Name (Inst : Class_Instance) return String is
      T : constant Target_Property_Access :=
        Target_Property_Access
   ...
   begin
      if Command = Constructor_Method then
         ...
         begin
            --  File is either a String or a GPS.File
            begin
               declare
                  File : constant Filesystem_String := Nth_Arg (Data, 3, "");
               begin
                  if File /= "" then
            ...
            begin
               ...
               if Data'Tag /= Python_Callback_Data'Class'Tag then
                  ...
                  --  or a list
                  Get_Param
                    (Python_Callback_Data'Class (Data), 5, Item, Success);
                  if not Success then
                     ...
                     declare
                        List   : List_Instance'Class := Nth_Arg (Data, 5);
         ...
         Extra_Args :=
           GNAT.OS_Lib.Argument_String_To_List (Nth_Arg (Data, 2, ""));
   ...
   begin
      ...
      Register_Command
        (Kernel.Scripts,
         "execute",
         Params  =>
           (Param ("main_name", Optional => True),   --  2
