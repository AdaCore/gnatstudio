...
package body Code_Coverage.GNATcov is
   ...
   function Coverage_Verbose_Message
     (Coverage : GNATcov_Line_Coverage) return String;
   ...
   is
      ...
      Location_Regexp  : constant Pattern_Matcher :=
        Compile ("\sat (\d+):(\d+)\s", Multiple_Lines);
      Location_Matches : Match_Array (0 .. 2);

      Metric_Regexp : constant Pattern_Matcher :=
        Compile ("^\d+% (.+) coverage \((\d+) out of (\d+)\)", Multiple_Lines);
      Level_Regexp  : constant Pattern_Matcher :=
        Compile ("^Coverage level: *(.*)$", Multiple_Lines);
      ...
      procedure Parse_Header (Header : String) is
         Data    : constant access Node_Coverage :=
           Node_Coverage (File_Node.Analysis_Data.Coverage_Data.all)'Access;
   ...
   begin
        ...
        new Line_Array
              (1
               ..
                 Natural'Value
                   (File_Contents
                      (Last_Line_Matches (1).First
                       .. Last_Line_Matches (1).Last)));
      ...
      loop
         ...
         begin
            ...
            loop
               Details_Last :=
                 Index (File_Contents.all, (1 => ASCII.LF), Details_First);
   ...
   begin
      if Text /= "" then
         ...
         Result.Associated_Command :=
           new Detail_Messages_Command'
   ...
   overriding
   function Is_Valid (Self : GNATcov_Line_Coverage) return Boolean is
   begin
   ...
   overriding
   function Print_Status (Self : GNATcov_Line_Coverage) return String is
   begin
   ...
   function Coverage_Verbose_Message
     (Coverage : GNATcov_Line_Coverage) return String is
   begin
      case Coverage.Status is
         ...
         when Branch_Taken          =>
            return
              -("The code for this line has been executed " & "branch taken.");
         ...
         when Exempted_Violated     =>
            return
              -("The code for this line has been exempted and a "
                & "violation has occurred.");

         when Exempted_Not_Violated =>
            return
              -("The code for this line has been exempted but no "
                & "violation has occurred.");
   ...
   function "=" (Left, Right : GPS.Editors.Editor_Mark'Class) return Boolean is
   ...
   begin
      ...
      then
         Process_Detailed_Messages
           (GNATcov_Line_Coverage (Coverage.all), Process'Access);
         Self.Added := True;
      end if;
   ...
   overriding
   procedure Primitive_Free (Self : in out Detail_Messages_Command) is
   begin
   ...
   begin
      for Mark of Self.Marks loop
         GPS_Editor_Buffer'Class (Buffer).Remove_Special_Lines (Mark, 1);
      end loop;
      Self.Added := False;
