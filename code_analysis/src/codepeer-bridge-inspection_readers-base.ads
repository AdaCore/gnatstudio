...
private package CodePeer.Bridge.Inspection_Readers.Base is
   ...
   type Base_Inspection_Reader (Kernel : not null GPS.Kernel.Kernel_Handle) is
   ...
   function Subprogram_Node
     (Self : Base_Inspection_Reader) return Code_Analysis.Subprogram_Access
   is abstract;
   ...
   procedure End_Message (Self : in out Base_Inspection_Reader);
   ...
   function Annotation_Category
     (Self : Base_Inspection_Reader'Class; Id : Natural)
      return Annotation_Category_Access;
   --  Return annotation category for given identifier.

private

   type Base_Inspection_Reader (Kernel : not null GPS.Kernel.Kernel_Handle) is
      abstract limited new Abstract_Inspection_Reader
   with record
