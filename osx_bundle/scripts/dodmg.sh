...
SIZE=`du -k -d 0 ${APP}.app | sed -e 's/\([0-9]*\).*/\1/'`
...
hdiutil create -srcfolder "${APP}.app" -volname "${APP} ${VERSION}" -fs HFS+ \
...
echo '
   tell application "Finder"
     tell disk "'${volume}'"
       ...
       set position of item "'${APP}'.app" of container window to {100, 170}
