...
The cross-reference engine expects to have fully up to date cross-reference
information generated by the compiler (the :file:`.ali` files).

If you start from such a state, and then for instance modify the spec for
an Ada package, then recompile that file only, any reference to entities
declared in that spec in other packages will not be found (until you
...
is still the same as before, or is a new one with the same name. So it always
err on the safe side, and assumes this is not the same entity.
