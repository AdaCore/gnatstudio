...
xref and partially compiled projects
------------------------------------

The cross-reference engine works best when it has fully up to date
cross-reference information generated by the compiler (the :file:`.ali` files).

If you start from such a state, and then for instance modify the spec or body
for an Ada package, then recompile that file only, any reference to entities
declared in that spec in other packages might no longer be found (until you
recompile those other packages, as :program:`gprbuild` would for instance).

This is because GPS has no way to know whether an entity 'Foo' in the spec
is still the same as before, or is a new one with the same name. It uses
an approximate algorithm where the references are only preserved if there
is still an entity with the same name at the exact same location in the
new version of the source. But if you had for instance a blank line in the
file, this changes the declaration line for all entities declared later
