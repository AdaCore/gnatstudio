...
package body GPS.LSP_Client.Dependency_Browers is
   ...
   type LSP_Dependency_Browser_Provider_Type is
     new Dependency_Browser_Provider_Interface
   with null record;
   ...
   is ("querying dependencies");

   function To_LSP_Kind
     (Kind : Dependency_Kind_Type) return ALS_ShowDependenciesKind;
   ...
   function To_LSP_Kind
     (Kind : Dependency_Kind_Type) return ALS_ShowDependenciesKind is
   begin
      case Kind is
         when Show_Imported  =>
            return Show_Imported;

         when Show_Importing =>
            return Show_Importing;
      end case;
   end To_LSP_Kind;
   ...
   begin
      if LSP_Ada_Support_Is_Active then
         Browsers.Dependency_Items.Set_Dependency_Browser_Provider (Provider);
      end if;
   end Register_Module;
