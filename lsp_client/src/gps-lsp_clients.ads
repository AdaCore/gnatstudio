...
private
   ...
   package Time_List is new
     Ada.Containers.Doubly_Linked_Lists (Ada.Calendar.Time);
   ...
   procedure Enqueue (Self : in out LSP_Client'Class; Item : in out Command);
   --  Put given command into the queue. If the server is shut down,
   --  Command.Request is destroyed and nullified instead.

   overriding
   procedure On_Error (Self : in out LSP_Client; Error : String);
   ...
   overriding
   procedure On_Started (Self : in out LSP_Client);
   --  Send initialization request on successful startup of the language
   --  server process.

   overriding
   procedure On_Finished (Self : in out LSP_Client);
