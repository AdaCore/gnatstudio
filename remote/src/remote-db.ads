...
package Remote.Db is
   ...
   type Synchronisation_Type is
     (Never, On_Request, Always, To_Remote, To_Local);
   ...
   type Machine_Type is
     new Gexpect.Machine_Type
     and GNATCOLL.Remote.Server_Record with private;
   type Machine_Access is access all Machine_Type'Class;

   overriding
   procedure Ref (Machine : in out Machine_Type);
   overriding
   procedure Unref (Machine : access Machine_Type);

   function New_Machine
     (Kernel : access Kernel_Handle_Record'Class; Nickname : String)
      return Machine_Type;
   overriding
   function Nickname (Machine : Machine_Type) return String;
   overriding
   function Network_Name (Machine : Machine_Type) return String;
   overriding
   function Access_Tool (Machine : Machine_Type) return String;
   overriding
   function Shell (Machine : Machine_Type) return String;
   overriding
   function Sync_Tool (Machine : Machine_Type) return String;
   overriding
   ...
   overriding
   function User_Name (Machine : Machine_Type) return String;
   overriding
   function Max_Nb_Connections (Machine : Machine_Type) return Natural;
   overriding
   function Timeout (Machine : Machine_Type) return Natural;
   overriding
   function Cr_Lf (Machine : Machine_Type) return Cr_Lf_Handling;
   overriding
   function Use_Dbg (Machine : Machine_Type) return Boolean;
   overriding
   procedure Dbg
     (Machine : access Machine_Type; Str : String; Mode : Mode_Type);
   procedure Set_Network_Name
     (Machine : in out Machine_Type; Network_Name : String);
   procedure Set_Access_Tool
     (Machine : in out Machine_Type; Access_Tool : String);
   procedure Set_Shell (Machine : in out Machine_Type; Shell : String);
   procedure Set_Sync_Tool (Machine : in out Machine_Type; Sync_Func : String);
   procedure Set_Extra_Init_Commands
     (Machine : in out Machine_Type; Cmds : String_List);
   overriding
   procedure Set_User_Name (Machine : in out Machine_Type; User_Name : String);
   procedure Set_Max_Nb_Connections
     (Machine : in out Machine_Type; Nb : Natural);
   procedure Set_Timeout (Machine : in out Machine_Type; MSeconds : Natural);
   procedure Set_Cr_Lf (Machine : in out Machine_Type; Cr_Lf : Cr_Lf_Handling);
   procedure Set_Use_Dbg (Machine : in out Machine_Type; Dbg : Boolean);

   --  Machine's shell properties
   overriding
   function Shell_Command (Machine : Machine_Type) return String;
   overriding
   ...
   function Shell_Configured_Prompt
     (Machine : Machine_Type) return Pattern_Matcher_Access;
   overriding
   function Shell_FS (Machine : Machine_Type) return FS_Type;
   overriding
   ...
   overriding
   function Shell_Cd_Cmd (Machine : Machine_Type) return String;
   overriding
   ...
   overriding
   function Get_Servers (Db : Remote_Db_Type) return String_List;
   ...
   overriding
   function Get_Server
     (Db : Remote_Db_Type; Nickname : String) return Gexpect.Machine_Access;
   ...
   function Get_Machine
     (Db : Remote_Db_Type; Nickname : String) return Machine_Access;
   --  Get the Machine according to its nickname.

   overriding
   function Nb_Mount_Points
     (Config : Remote_Db_Type; Nickname : String) return Natural;
   ...
   function Get_Mount_Points
     (Config : Remote_Db_Type; Nickname : String) return Mount_Point_Array;
   ...
   function Is_Sys_Default
     (Config : Remote_Db_Type; Nickname : String) return Boolean;
   function Has_Sys_Default
     (Config : Remote_Db_Type; Nickname : String) return Boolean;
   function Get_Sys_Default
     (Config : Remote_Db_Type; Nickname : String) return Machine_Access;
   procedure Add_Or_Replace
     (Config    : access Remote_Db_Type;
      Machine   : Machine_Access;
      Is_System : Boolean := False);
   procedure Remove (Config : access Remote_Db_Type; Nickname : String);
