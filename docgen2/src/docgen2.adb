...
package body Docgen2 is
   ...
   overriding function Name
     (Command : access Docgen_Command) return String;
   ...
   overriding procedure Free
     (X : in out Docgen_Command);
   --  See inherited for documentation

   --  Local package containing subsidiary routines of Docgen_Command
   package Local is
   ...
   is
      ...
      Lock : Database_Lock := Database.Freeze;
   begin
      ...
      case Command.State is
         when Analysis_Setup =>
            ...
            Local.Generate_Support_Files (Docgen_Object (Command));
         ...
         when Update_Xrefs =>
            ...
            File := Element (Command.File_Index);
            ...
            if Command.Options.Process_Up_To_Date_Only
              and then not Database.Is_Up_To_Date (File)
            then
               Insert
                 (Command.Kernel,
         ...
         when Analyse_Files =>
            ...
            begin
               ...
               begin
                  ...
                  --          (Lang_Handler,
                  --           Command.Virtual_Files (Command.File_Index));
                  --
         ...
         when Analyse_File_Constructs =>
            ...
            if Has_Next_Construct (Command.Analysis_Ctxt) then
               Local.Analyse_Construct (Docgen_Object (Command));
            ...
            --  child e_info to move comment when appropriate
               ...
               loop
                  Exit_Scope (Command.Analysis_Ctxt);
               end loop;
         ...
         when Gen_Doc_Setup =>
            ...
            Local.Generate_Comments (Docgen_Object (Command));
            --  Generate all cross-refs
            Trace (Me, "Generate all Cross-Refs");
            Local.Generate_Xrefs (Docgen_Object (Command));
         ...
         when Gen_Doc_Annotated_Src =>
            ...
            if Active (Me) then
               Trace
                 (Me, "Generate annotated source for " & (+File.Base_Name));
            end if;
         ...
         when Gen_Doc_Tear_Down =>
            ...
            Local.Generate_User_Indexes (Docgen_Object (Command));
            Local.Generate_Trees (Docgen_Object (Command));
            Local.Generate_Global_Index (Docgen_Object (Command));
   ...
   package body Local is
      ...
      begin
         ...
         --  Now add custom ones
         Cursor := List.First;
         while Docgen2.Scripts.Custom_CSS_File_Vectors.Has_Element (Cursor)
         loop
         ...
         for J in Cmd.Custom_Files.First_Index .. Cmd.Custom_Files.Last_Index
         loop
            Append (Names_Tag,
              To_String (Cmd.Custom_Files.Element (J).Name));
            Append (Files_Tag,
              To_String (Cmd.Custom_Files.Element (J).Filename));
         end loop;
      ...
      procedure Analyse_Construct (Cmd : Docgen_Object) is
         ...
         function Get_Entity
           (Construct : access Simple_Construct_Information)
            return Entity_Info;
         --  Retrieve and decorate the entity associated with Construct (if
         ...
         is
            ...
            begin
               ...
               E_Info :=
                 new Entity_Info_Record (Category => To_Category (Cat));
            ...
            function Create_Xref
              (E             : Root_Entity'Class;
               Entity        : Root_Entity'Class;
               Use_Full_Name : Boolean := False) return Cross_Ref
            is
               Loc : General_Location;
            ...
            begin
               ...
               Loc := Get_Declaration (E).Loc;
               Loc_E := Get_Declaration (Entity).Loc;
               ...
               then
                  return Create_Xref (Get_Name (E), Loc);
               else
                  return Create_Xref (Qualified_Name (E), Loc);
               end if;
            end Create_Xref;
            ...
            Entity : Root_Entity'Class := Docgen2.Utils.Get_Entity
              (Cmd.Kernel,
         ...
         begin
            ...
            if Entity = No_Root_Entity then
               ...
               E_Info.Name := GNATCOLL.Symbols.Find
                 (Fullnames, Qualified_Name (Entity));
               E_Info.Short_Name := GNATCOLL.Symbols.Find
                 (Fullnames, Get_Name (Entity));
            ...
            if Is_Container (Entity) then
               E_Info.Location.Body_Loc := Get_Body (Entity);
            end if;
            ...
            if Construct.Category not in Namespace_Category then
              ...
              and then Entity /= No_Root_Entity
            then
               Set_Pkg_Printout
                 (Construct   => Construct.all,
                  Entity      => Entity,
                  File_Buffer => Get_File_Buffer (Context),
                  E_Info      => E_Info);
            end if;
            ...
            begin
               ...
               then
                  ...
                  declare
                     Calls_Iter : Calls_Iterator'Class :=
                       Get_All_Called_Entities (Entity);
                  begin
                     while not At_End (Calls_Iter) loop
                        declare
                           Called_E : constant Root_Entity'Class :=
                             Get (Calls_Iter);
                        begin
            ...
            E_Info.Is_Abstract := Is_Abstract (Entity);
            E_Info.Is_Generic := Is_Generic (Entity);
            E_Info.Is_Renaming := Renaming_Of (Entity) /= No_Root_Entity;
            E_Info.Is_Instantiation :=
              Instance_Of (Entity) /= No_Root_Entity;
            ...
            if E_Info.Is_Generic then
               declare
                  ...
                  Params       : Xref.Entity_Array :=
                    Formal_Parameters (Entity);
                  Param_EInfo  : Entity_Info;
               begin
                  for P in Params'Range loop
                     declare
                        Param_Entity : constant Root_Entity'Class :=
                          Params (P).all;
                     begin
                        Param_EInfo := Create_EInfo
                          (Cat_Parameter,
                           Get_Declaration (Param_Entity).Loc,
                           Name);
                        Param_EInfo.Name := Db.Symbols.Find
                          (Get_Name (Param_Entity));
                        ...
                        --  ??? need a better handling of formal
                        --  parameters types
                        Param_EInfo.Parameter_Type := Create_Xref
                          (Name => Get_Display_Kind (Param_Entity),
                           Loc  => No_Location,
                           Xref => Param_EInfo);
            ...
            if E_Info.Is_Renaming then
               declare
                  Renamed_Entity : constant Root_Entity'Class :=
                                     Renaming_Of (Entity);
               begin
            ...
            if E_Info.Is_Instantiation then
               declare
                  Instantiated_Entity : constant Root_Entity'Class :=
                    Instance_Of (Entity);
               begin
            ...
            declare
               Body_Location : constant General_Location := Get_Body (Entity);
            ...
            case E_Info.Category is
                        ...
                        if E_Info.Is_Generic then
                           ...
                           begin
                              ...
                              while Entity_Info_List.Has_Element (Cursor) loop
                                 Entity_Info_List.Element
                                   (Cursor).Location.Pkg_Nb := E_Info.Pkg_Nb;
               ...
               when Cat_Class =>
                  ...
                  declare
                     Parents : Xref.Entity_Array :=
                       Parent_Types (Entity, Recursive => True);
                  begin
                     for J in Parents'Range loop
                        --  Use full name, as Parents is used in the
                        --  inheritance tree and we want fully qualified
                        --  names there.
                        E_Info.Parents.Append
                          (Create_Xref (Parents (J).all, Entity, True));
                     end loop;
                  ...
                  --  Get known children
                  declare
                     Children : Xref.Entity_Array := Child_Types
                       (Entity, Recursive => False);
                  begin
                     for C in Children'Range loop
                        if Children (C).all /= No_Root_Entity then
                           E_Info.Class_Children.Append
                             (Create_Xref (Children (C).all, Entity, True));
                        end if;
                     end loop;
                  ...
                  declare
                     Methods : Xref.Entity_Array :=
                       Xref.Methods (Entity, Include_Inherited => False);
                     Op_Xref          : Cross_Ref;
                     ...
                     List : array (Methods'Range) of Primop_Info;

                  begin
                     --  Init the list of primitive operations
                     for J in List'Range loop
                        List (J) := (new Root_Entity'Class'(Methods (J).all),
                                     False);
                     end loop;
                     ...
                     for J in List'Range loop
                        for K in J + 1 .. List'Last loop
                           if Overrides
                             (List (J).Entity.all) = List (K).Entity.all
                           then
                     ...
                     for J in List'Range loop
                        if not List (J).Overriden then
                           ...
                           declare
                              E_Overriden : constant Root_Entity'Class :=
                                Overrides (List (J).Entity.all);
                           begin
                              if E_Overriden /= No_Root_Entity then
               ...
               when Cat_Variable =>
                  --  Get its type
                  declare
                     Type_Entity : Root_Entity'Class := Get_Type_Of (Entity);
                  begin
                     if Type_Entity = No_Root_Entity then
                     ...
                     if Type_Entity = No_Root_Entity then
                        Type_Entity := Returned_Type (Entity);
                     end if;
               ...
               when others =>
                  ...
                  --  Cat_Literal (do we want to output anything for a
                  --  literal ?)
                  null;
         ...
         begin
            return
              Construct.Category in Dependency_Category
                or else Construct.Category
                          in Cat_Exception_Handler .. Cat_Pragma
               ...
               case E_Info.Category is
                  ...
                  when Cat_Task | Cat_Protected =>
                     Enter_Scope
                       (Context => Context,
                        Parent => E_Info,
                        Entity => Context_Elem.Pkg_Entity);
                     return;
      ...
      begin
         ...
         while Nxt <= Doc'Last loop
            ...
            then
               ...
               if Docgen.EInfos_By_Name.Contains
                    (Doc (Short_N .. Nxt - 1))
               then
                  declare
                  ...
                  begin
                     ...
                     for K in Vect.First_Index .. Vect.Last_Index loop
                        if Short_N = Idx
                          and then Vect.Element (K).Location.Spec_Loc.File
                                      = File
                        then
                           E := Vect.Element (K);
                           Done := True;
                       ...
                       (Doc (Idx .. Nxt - 1),
                        Docgen.Backend.To_Href
                          (Spec_Location_Image (E),
                           E.Location.Spec_Loc.File.Base_Name,
                           E.Location.Pkg_Nb),
                        Doc (Idx .. Nxt - 1)));
      ...
      is
         ...
         begin
            ...
            if Entity not in Identifier_Entity then
               ...
               declare
                  Decl_Entity  : constant Root_Entity'Class :=
               ...
               begin
                  ...
                  if Decl_Entity /= No_Root_Entity then
                     Loc := Get_Declaration (Decl_Entity).Loc;
                  ...
                  if EInfo /= null then
                     ...
                     begin
                        ...
                        if Decl_Entity /= No_Root_Entity
                          and then Loc.File = File
                          and then Loc.Line = Sloc_Start.Line
                          and then Loc.Column =
                            GNATCOLL.Xref.Visible_Column (Sloc_Start.Column)
                        then
                           --  We are at the spec declaration: do not provide
                           --  links to ourselves.
                           To_Spec := False;
                        end if;

                        if  EInfo.Location.Body_Loc.File = File
                          and then EInfo.Location.Body_Loc.Line
                            = Sloc_Start.Line
                        then
                           --  We are at the body location
                           To_Body := False;

                        elsif EInfo.Location.Body_Loc.File = No_File then
                           To_Body := False;
                        end if;
      ...
      begin
         ...
         Insert
           (Translation, Assoc ("Virtual_File", +File.Base_Name));
         Insert
      ...
      procedure Generate_Comments (Cmd : Docgen_Object) is
         ...
         begin
            Cmd.Current_File := Elem.Location.Spec_Loc.File;
            Analyse_Comment
              (Elem.Description,
               Cmd,
               ...
               Cmd.Backend.To_Href
                 (Spec_Location_Image (Elem),
                  Elem.Location.Spec_Loc.File.Base_Name,
      ...
      begin
         ...
         while Entity_Info_Map.Has_Element (Cursor) loop
            Entity_Info_Map.Update_Element
              (Cmd.EInfos, Cursor, Internal'Access);
      ...
      is
         ...
         --  Get name from E, and formats it to reflect its attribute
         --  (abstract, generic)
         ...
         begin
            for J in Command.Src_Files.First_Index
                       .. Command.Src_Files.Last_Index
            loop
         ...
         begin
            ...
            if E_Info.Location.Body_Loc /= No_Location then
               Found := Is_In_Prj (E_Info.Location.Body_Loc.File);
            end if;
            ...
            if Found then
               Append
                 (Tags.Body_Src_Tag,
                  Command.Backend.To_Href
                    (Location =>
                       Command.Backend.Line_Image
                         (E_Info.Location.Body_Loc.Line),
                     Src_File =>
            ...
            for J in E_Info.References.First_Index
                       .. E_Info.References.Last_Index
            loop
               declare
                  Loc      : constant General_Location :=
                               Get_Location (E_Info.References.Element (J));
                  Src_File : constant GNATCOLL.VFS.Virtual_File := Loc.File;
               begin
                  ...
                  if E_Info.Called.Element (J).Xref /= null
                    and then
                      (E_Info.Called.Element (J).Xref.Category = Cat_Package
                         or else
                       E_Info.Called.Element (J).Xref.Category = Cat_File)
                  then
         ...
         begin
            Insert (Translation,
              Assoc (Tag_Name, CI.Name_Tag));
            Insert (Translation,
              Assoc (Tag_Name & "_SRC", CI.Src_Tag));
            Insert (Translation,
              Assoc (Tag_Name & "_BODY_SRC", CI.Body_Src_Tag));
            Insert (Translation,
              Assoc (Tag_Name & "_PRINTOUT", CI.Printout_Tag));
            Insert (Translation,
            ...
            Insert (Translation,
              Assoc (Tag_Name & "_LOC", CI.Loc_Tag));
            Insert (Translation,
              Assoc (Tag_Name & "_INSTANTIATION", CI.Instantiation_Tag));
            Insert (Translation,
              Assoc (Tag_Name & "_RENAMES", CI.Renames_Tag));
            Insert (Translation,
              Assoc (Tag_Name & "_CAT", CI.Cat_Tag));
         ...
         procedure Format_Printout (E_Info : Entity_Info) is
            ...
            begin
               ...
               if Active (DOCGEN_V3) and then Entity = Comment_Text then
                  ...
                  --  If entity is an identifier or a partial identifier, then
                  --  try to find its corresponding Entity_Info
                  ...
                  then
                     ...
                     --  EInfo should now contain the Entity_Info corresponding
         ...
         begin
            if E_Info.Printout /= null then
               ...
               Parse_Entities (Lang,
                 E_Info.Printout.all, CB'Unrestricted_Access);
      ...
      begin
         ...
         if E_Info.Category = Cat_File then
            ...
            --  Set main unit name
            Insert (Translation, Assoc ("Virtual_File", Get_Name (E_Info)));
            ...
            begin
               ...
               while Entity_Info_List.Has_Element (Cursor) loop
                  ...
                  Name_Str :=
                    To_Unbounded_String
                      (Command.Backend.Gen_Tag
                         (Identifier_Text, Get_Name (Param)));
         ...
         while Entity_Info_List.Has_Element (Cursor) loop
               ...
               begin
                  ...
                  loop
                     ...
                     then
                        Append (Parent_Tag,
                          Gen_Href
                            (Command.Backend, Xref, True, False, False));
                     end if;
                  ...
                  loop
                     ...
                     then
                        ...
                        Prim_Op_Str :=
                          To_Unbounded_String
                            (Gen_Href
                               (Command.Backend, Xref, True, True, False));
            ...
            then
               ...
               begin
                  ...
                  Entry_Cursor :=
                    Entity_Info_List.First (Child_EInfo.Children);
                         ...
                         (Get (E_Entry.Short_Name).all,
                          Command.Backend.To_Href
                            (Spec_Location_Image (E_Entry),
                             E_Entry.Location.Spec_Loc.File.Base_Name,
                             E_Entry.Location.Pkg_Nb),
                          Get (E_Entry.Short_Name).all));

                     Append (Task_Entry_Parent,
                       Get (Child_EInfo.Short_Name).all);
         ...
         if E_Info.Location.Body_Loc /= No_Location
           and then E_Info.Location.Body_Loc.File /= No_File
         then
            declare
               File  : constant GNATCOLL.VFS.Virtual_File :=
                         E_Info.Location.Body_Loc.File;
            begin
         ...
         Insert (Translation,
           Assoc ("PKG_FULL_LINK", Pkg_Full_Link));
         Insert (Translation,
           Assoc ("PKG_GENERIC_PARAMETERS", Pkg_Gen_Params));
         Insert (Translation,
           Assoc ("PKG_GENERIC_PARAMETERS_LOC", Pkg_Gen_Params_Loc));

         Insert_Common_Informations ("CLASS", Class_CI);
         Insert (Translation,
           Assoc ("CLASS_PARENTS", Class_Parents));
         Insert (Translation,
           Assoc ("CLASS_CHILDREN", Class_Children));
         Insert (Translation,
           Assoc ("CLASS_PRIMITIVES", Class_Primitives));

         Insert_Common_Informations ("TASK", Task_CI);
         Insert (Translation,
           Assoc ("TASK_TYPE", Task_Type));
         Insert (Translation,
           Assoc ("TASK_IS_TYPE", Task_Is_Type));

         Insert_Common_Informations ("TASK_ENTRY", Task_Entry_CI);
         Insert (Translation,
           Assoc ("TASK_ENTRIES_HREF", Task_Entry_Hrefs));
         Insert (Translation,
           Assoc ("TASK_ENTRY_PARENT", Task_Entry_Parent));
         Insert (Translation,
      ...
      procedure Generate_Global_Index (Cmd : Docgen_Object) is
         Tmpl         : constant Virtual_File :=
      ...
      begin
         ...
         for J in Local_List'Range loop
            ...
            for K in Local_List (J).First_Index .. Local_List (J).Last_Index
            loop
         ...
         Last_Letter := ' ';
         for J in Cmd.Src_Files.First_Index .. Cmd.Src_Files.Last_Index + 1
         loop
      ...
      begin
         ...
         --  Then copy individual user-defined css files into this support
         --  subdir
         Cursor := List.First;
         while Docgen2.Scripts.Custom_CSS_File_Vectors.Has_Element (Cursor)
         loop
      ...
      procedure Generate_Trees (Cmd : Docgen_Object) is
         Tmpl          : constant Virtual_File :=
                           Cmd.Backend.Get_Template
         ...
         begin
            ...
            Append (Tree_Tag,
              Gen_Href (Cmd.Backend, Class, True, False, False, Name));
      ...
      begin
         ...
         loop
            declare
               Obj : Custom_File_Record renames
                       Command.Custom_Files.Element (J);
            begin
               Insert
      ...
      procedure Generate_Xrefs (Cmd : Docgen_Object) is
         Xref_Cursor  : Cross_Ref_List.Cursor;
         Xref_Node    : Cross_Ref;
         EInfo_Cursor : Entity_Info_Map.Cursor;
      ...
      function Gen_Href
        (Backend : Backend_Handle;
         ...
         Name    : String) return String
      is
         type Href_Kind is (API_Href, Spec_Href, Body_Href);
         ...
         begin
            ...
            when API_Href =>
               ...
               return Backend.To_Href
                 (Spec_Location_Image (EInfo),
                  EInfo.Location.Spec_Loc.File.Base_Name,
            ...
            when Spec_Href =>
               ...
               return Backend.To_Href
                 (Backend.Line_Image (EInfo.Location.Spec_Loc.Line),
                  "src_" & EInfo.Location.Spec_Loc.File.Base_Name,
                  Pkg_Nb   => 1);
