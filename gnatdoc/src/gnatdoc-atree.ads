...
--       Defined in all entities. For subprograms and entries returns their
--       first formal parameter. If not available returns No_Entity.
...
--    Has_Scope
--       Defined in all entities. This attribute has three possible values:
--       'Yes' if this entity defines an scope; 'No' if it does not define an
--       scope; 'Unknown' if it has not been yet computed by the frontend.
--       This attribute was added to workaround the GNATcoll decoration of
--       record type entities which made unreliable the computation of this
--       high level attribute; now it is computed by the frontend, and after
--       set it is never changed (to provide a consistent value).
...
--    Is_Container (LL)
--       Defined in all entities. True for entities that define an scope. This
--       attribute is not reliable in all entities and it is adjusted by the
--       frontend.
...
--       level declarations. This attribute is not reliable in all entities
--       and it is adjusted by the frontend.
...
private package GNATdoc.Atree is
   ...
   type T_Scope_Value is (Unknown, Yes, No);

   function Has_Scope
     (E : Entity_Id) return T_Scope_Value;
   --  Return Yes if the entity has scope; return No if it does not has scope;
   --  return Unknown if it has not been yet computed by the frontend. Used to
   --  workaround the GNATcoll decoration of record type entities which makes
   --  unreliable the computation of this high level attribute.
   ...
   procedure Set_Has_Scope
     (E : Entity_Id; Value : T_Scope_Value);
   ...
   package LL is
      ...
      procedure Clear_Etype (E : Entity_Id);
      --  Set the Etype of E to No_Entity. Used to workaround GNATColl.Xref
      --  decoration (for backward compatibility).
      ...
      --  identify the second entity generated by the C/C++ compiler for named
      --  typedef structs (the compiler generates two entites in the LI file
      --  with the same name).

      procedure Set_Has_Methods
        (E : Entity_Id; Value : Boolean);
      --  Used to workaround GNATColl.Xref wrong decoration.
   ...
   private
      ...
      pragma Inline (Set_Has_Methods);
      pragma Inline (Set_Is_Abstract);
      pragma Inline (Set_Is_Container);
      pragma Inline (Set_Is_Global);
...
private
   ...
   pragma Inline (Get_First_Formal);
