...
--     "control" keys) causes GNAT Studio to enter a mode called hyper-mode.
--   . Releasing the key causes GNAT Studio to leave the hyper-mode.
--
--  Any widget in GNAT Studio can decide to react to hyper-mode by changing
--  their behavior when hyper-mode is active. They do this through a call to
