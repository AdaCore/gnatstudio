...
package Language.Abstract_Construct_Tree is
   ...
   type Abstract_Construct_Tree_Iterator is new Semantic_Tree_Iterator
   with record
   ...
   overriding
   procedure Next (Self : in out Abstract_Construct_Tree_Iterator);
   ...
   overriding
   procedure Update (Self : in out Abstract_Construct_Tree);

   overriding
   procedure Update_Async (Self : in out Abstract_Construct_Tree);

   overriding
   function Is_Ready (Self : Abstract_Construct_Tree) return Boolean
   is (True);

   overriding
   function Category (Self : Construct_Node) return Language_Category;

   overriding
   function Parent (Self : Construct_Node) return Semantic_Node'Class;

   overriding
   function Children (Self : Construct_Node) return Semantic_Node_Array'Class;

   overriding
   function First_Child (Self : Construct_Node) return Semantic_Node'Class;

   overriding
   function Name (Self : Construct_Node) return GNATCOLL.Symbols.Symbol;

   overriding
   function Profile
     (Self : Construct_Node; Show_Param_Names : Boolean)
      return GNATCOLL.Symbols.Symbol;

   overriding
   function Is_Valid (Self : Construct_Node) return Boolean
   is (True);

   overriding
   function Definition (Self : Construct_Node) return Semantic_Node'Class;

   overriding
   function Sloc_Def (Self : Construct_Node) return Sloc_T;

   overriding
   function Sloc_Start (Self : Construct_Node) return Sloc_T;

   overriding
   function Sloc_End (Self : Construct_Node) return Sloc_T;

   overriding
   function Get_Hash (Self : Construct_Node) return Hash_Type;

   overriding
   function File (Self : Construct_Node) return GNATCOLL.VFS.Virtual_File;

   overriding
   function Is_Declaration (Self : Construct_Node) return Boolean;

   overriding
   function Visibility (Self : Construct_Node) return Construct_Visibility;

   overriding
   function Unique_Id (Self : Construct_Node) return GNATCOLL.Symbols.Symbol;

   overriding
   function Documentation_Header (Self : Construct_Node) return String;
...
private
   ...
   package Construct_Node_Vectors is new
     Ada.Containers.Vectors (Positive, Construct_Node);
   ...
   is (Self.Nodes.Element (Index));

   overriding
   function Length (Self : Construct_Node_Array) return Natural
   is (Natural (Self.Nodes.Length));
