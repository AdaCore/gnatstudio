...
package body GPS.Kernel.Properties is
   ...
   is
      ...
      S   : Integer := Str'First;
   begin
      Trace (Me, "MANU Writing to string stream " & Item'Length'Img);
   ...
   begin
      Property_Record'Class'Write
        (Str'Access, Property_Record'Class (Property.all));

      return new Node'
        (Tag        => new String'("stream"),
         Attributes => null,
         --  ??? Would be better to use a <![CDATA[...]]> section, but special
         --  characters get quoted by the glib-xml. Perhaps when we are using
         --  XML/Ada to write XML streams ?
         Value      => new String'(To_String (Str.Str)),
         Parent     => null,
         Child      => null,
         Next       => null,
         Specific_Data => 1);
   ...
   is
      Str : aliased String_Stream_Type;
   begin
      Open (Str, From.Value.all);
      Property_Record'Class'Read
        (Str'Access, Property_Record'Class (Property));
   end Load;
