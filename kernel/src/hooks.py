...
class Debounce_Hook_Type(Hook_Type):
    ...
    def __init__(
        ...
        """
        Hook_Type.__init__(
            self, params, None, None, -1, descr, internal_run, override_run_from_python
        )
...
class Param(object):
    ...
    def __init__(
        self, name, type, default=None, descr="", inpython=True, asynch_key=None
    ):
        self.name = name
        self.type = type
        self.descr = descr
...
types = {
    ...
    "Task": Mapping(
        ...
        toada="Get_Data (Data.Nth_Arg (%(idx)d), " + 'K.Scripts.New_Class ("Task"))',
    ...
    "Character": Mapping(ada="Glib.Gunichar", python=None),
    "File_Status": Mapping(
    ...
    "Visible_Column": Mapping(
        ...
        toada="GNATCOLL.Xref.Visible_Column" " (Integer'(Data.Nth_Arg(%(idx)d)))",
        withs=["GNATCOLL.Xref"],
    ),
    "Any_Type": Mapping(
        ada="GNATCOLL.Any_Types.Any_Type", python=None, withs=["GNATCOLL.Any_Types"]
    ),
    "Child_Group": Mapping(
    ...
    "Distant_Server": Mapping(
        ...
        python="str",
        toada="Remote.Distant_Server_Type'Value (String'(Data.Nth_Arg " + "(%(idx)d)))",
...
hook_types = {
    "simple_hooks": Hook_Type([Param("name", "__hookname__")]),
    "string_hooks": Hook_Type([Param("name", "__hookname__"), Param("str", "String")]),
    "project_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("project", "Project")]
    ),
    "file_hooks": Hook_Type([Param("name", "__hookname__"), Param("file", "File")]),
    "file2_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("file", "File"), Param("file2", "File")]
    ),
    "file_return_boolean_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("file", "File")],
        returns="Boolean",
        return_default="False",
    ),  # stop when one function returns True
...
background task or process""",
    ...
    "string_return_any_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("str", "String")],
        returns="Any_Type",
    ...
    "return_boolean_hooks": Hook_Type(
        [Param("name", "__hookname__")], returns="Boolean", return_default="True"
    ...
    "context_hooks": Debounce_Hook_Type(
        [Param("name", "__hookname__"), Param("context", "Context")],
        debounce=400,
        descr="Hooks that take a context as parameter",
    ),
    "mdi_child_hooks": Debounce_Hook_Type(
        [Param("name", "__hookname__"), Param("child", "MDI_Child")],
        debounce=400,
    ...
    "preferences_hooks": Debounce_Hook_Type(
        [Param("name", "__hookname__"), Param("pref", "Preference", default="null")],
        debounce=400,
    ),
...
If -1, all editors for this file will be closed instead""",
            ...
            Param(
                "group",
                "Child_Group",
                default="Gtkada.MDI.Group_Default",
                inpython=False,
            ),
            Param(
                "initial_position",
    ...
    "line_info_hooks": Hook_Type(
        [
            ...
            Param("info", "Line_Info_Data", inpython=False, default="null"),
...
executed command line as list of arguments""",
    ...
    "marker_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("location", "Location")]
    ),
    ...
    "debugger_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("debugger", "Debugger")]
    ),
    ...
    "vcs_refresh_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("is_file_saved", "Boolean")]
    ),
    "language_server_lifecycle_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("language", "String")]
    ),
    ...
    "DAP_method_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("method", "String")]
    ),
    "DAP_event_hooks": Hook_Type(
        [Param("name", "__hookname__"), Param("event", "String")]
    ),
    "DAP_id_hooks": Hook_Type([Param("name", "__hookname__"), Param("id", "Integer")]),
}
...
""",
    ...
    Hook(
        ...
        "language_server_lifecycle_hooks",
        descr="""
...
"""
    ...
    for type_name in sorted(hook_types.keys()):
        ...
        plist = [", %s" % p.name.title() for p in t.params if p.show_in_ada()]
        ...
        pset = [
            """
                  Data.Set_Nth_Arg
                     (%d, %s);"""
            % (index + 1, types[p.type].topython % {"ada": p.name.title()})
        ...
        param_descr = "\n".join(
            "   --  %s : %s"
            % (p.name.title(), p.descr.strip().replace("\n", "\n   --    "))
            for p in t.params
            if p.descr
        )
        if param_descr:
        ...
        if isinstance(t, Debounce_Hook_Type):
            asynch_plist = [
                ", Data.%s" % p.name.title() for p in t.params if p.show_in_ada()
            ]

            compare_keys = [
                "Data.%s = %s" % (p.name.title(), p.name.title())
                for p in t.params
                if p.asynch_key
            ]

            compare = [
                "%s%s" % (" and then " if idx > 0 else "", p)
      ...
      if Active (Me) then
         Trace (Me, "Default handling for " & Name (Self));
            ...
            subst["run_from_python_var"] = ""
            subst["run_from_python_body"] = (
                """Self.Run   --  not dispatching
         (Kernel   => K%(p_in_run)s);"""
                % subst
            )
...
"""
        ...
        for hook in hooks:
            if hook.type == type_name:
                f.write(
                    """   N_%(name)s : aliased constant String := "%(base)s";
...
"""
    ...
    for h in hooks:
        ...
        b.write(
            "      %s.Name := To_Unbounded_String (N_%s);\n" % (varname, h.name.title())
        )
...
def generate_doc():
    ...
    """
    f = open("../../docs/users_guide/GPS/generated_hooks.py", "w", newline="\n")
    f.write(
        '''
import GPS
    ...
    for h in hooks:
        ...
        params = [
            ...
            % {
                ...
                "doc": p_param(p),
                "default": " (default: %s)" % p.default if p.default else "",
                "name": p.name,
            }
