...
package body GPS.Kernel.Macros is
   ...
   function Project_From_Param
     (Param : String; Context : GPS.Kernel.Selection_Context)
      return Project_Type;
   ...
   is
      Req : Requirements :=
        (if Filter /= null
         then Filter.Requires
         else (Project => ' ', others => False));
      ...
      begin
         if Param = "f" or else Param = "F" or else Param = "fk" then
            Req.File := True;

         elsif Param = "d" or else Param = "dk" then
            Req.Directory := True;

         elsif Param = "e" or else Param = "ek" then
            Req.Entity := True;
   ...
   begin
      ...
      if Param = "d" then
         if Has_Directory_Information (Context) then
            return
              String_Utils.Protect
                (+To_Remote
                    (Directory_Information (Context), Get_Nickname (Server))
                    .Full_Name,
         ...
         if Has_Entity_Name_Information (Context) then
            if Get_Entity (Context) /= No_Root_Entity then
               ...
               if Is_Fuzzy (Get_Entity (Context)) then
                  return Entity_Name_Information (Context) & " (best guess)";
               else
                  return Entity_Name_Information (Context);
               end if;
            end if;
         ...
         return
           Create_From_Dir (Get_Kernel (Context).Get_System_Dir, "bin")
             .Display_Full_Name;
   ...
   begin
      return
        ...
        & (if Filter.Requires.Project /= ' '
           then "project " & Filter.Requires.Project
           else "");
   end Get_Debug_Name;
   ...
   is
      ...
      Start, Last     : Integer;
   begin
      if Filter.Requires.Project = 'p' or else Filter.Requires.Project = 'P'
      then
         Project :=
           Project_From_Param (Filter.Requires.Project & ' ', Context);
         if Project = No_Project then
            return False;
         end if;
      end if;

      if Filter.Requires.File and then not Has_File_Information (Context) then
         return False;
      end if;
      ...
      if Filter.Requires.Line and then not Has_Line_Information (Context) then
         return False;
      end if;

      if Filter.Requires.Column and then not Has_Column_Information (Context)
      then
         return False;
      end if;
