...
package Interactive_Consoles is
   ...
   --  Mode is used for highlighting, based on the user's color preferences.
   ...
   --  If the handler returns True, GNAT Studio will do no further processing
   --  on the key and simply ignore it. The default is to just return False,
   --  ie let the console do its own high-level management of user input.
...
private
   ...
   type Reserved_Tag_Kinds is
     (Uneditable_Tag,
      ...
      --  A tag used to identify messages that were generated by GNAT Studio
      --  but not by a user comment.
      Error_Messages_Tag,
      --  A tag used to distinguish error messages inserted in the console
      Hyper_Links_Tag,
