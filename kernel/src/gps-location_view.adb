...
package body GPS.Location_View is
   ...
   package Expansion_Request_Vectors is new
     Ada.Containers.Vectors (Positive, Expansion_Request);
   ...
   function Initialize
     (Self : access Location_View_Record'Class) return Gtk_Widget;
   --  Creates the locations view, and returns the focus widget

   package Location_Views is new
   ...
   package Select_Function_With_View is new
     Set_Select_Function_User_Data (Location_View);

   function Selection_Function
     (Selection               :
        not null access Gtk_Tree_Selection_Record'Class;
   ...
   type Selection_To_String_Command (Action : Selection_To_String_Action) is
   ...
   type View_Manager (Kernel : not null access Kernel_Handle_Record'Class) is
     new Abstract_Listener
   with null record;
   ...
   Set_Sorting_Hint_Parameters : constant Cst_Argument_List :=
     (1 => Category_Cst'Access, 2 => Hint_Cst'Access);
   ...
   procedure On_Row_Deleted (Self : access Location_View_Record'Class);
   ...
   package Location_View_Callbacks is new
     Gtk.Handlers.Callback (Location_View_Record);
   ...
   function Format_Message
     (Message : Message_Access; Add_File_Name : Boolean := True) return String;
   ...
   is
      Auto : constant Boolean :=
        Allow_Auto_Jump_To_First and then Auto_Jump_To_First.Get_Pref;
   begin
      Expand_Category
   ...
   begin
      Location_Views.Child_From_View
        (Location_Views.Get_Or_Create_View (Self.Kernel, Focus => False))
        .Highlight_Child;
   end Message_Added;
   ...
   begin
      if V /= null then
         Expand_Or_Collapse_Selected_Rows
           (Tree => V.View, Command => Self.Command);
      end if;
   ...
   is
      ...
      function Next (Path : Gtk_Tree_Path) return Boolean is
         Success           : Boolean := True;
         ...
         if Same_Weight then
            ...
            begin
               loop
                  ...
                  Reach_Limit :=
                    not Success or else Get_Iter (Model, Path) = Null_Iter;
      ...
      function Down_Path (Path : Gtk_Tree_Path) return Boolean is
         Success : Boolean := True;
      begin
         Path.Down;
   ...
   begin
      ...
      if N_Selected = 1 then
         ...
         begin
            ...
            if Path.Get_Depth >= 3 then
               Message :=
                 Get_Message (Model, Get_Iter (Model, Path), -Message_Column);
         ...
         declare
            ...
            Messages       :
              GPS.Kernel.Messages.Message_Array (1 .. N_Selected);
         ...
         begin
            ...
            if Messages_Index /= 0 then
               ...
               if Only_Messages and then File /= GNATCOLL.VFS.No_File then
   ...
   begin
      Set_Pref
        (Sort_By_Subcategory,
         K.Get_Preferences,
         not Sort_By_Subcategory.Get_Pref);
      return Commands.Success;
   end Execute;
   ...
   begin
      Set_Pref
        (Location_Only_High_Messages,
         K.Get_Preferences,
         not Location_Only_High_Messages.Get_Pref);
      return Commands.Success;
   end Execute;
   ...
   begin
      ...
      if Iter = Null_Iter then
         ...
         while Category_Iter /= Null_Iter loop
            exit when
              Get_String (Model, Category_Iter, -Category_Column)
              = "Builder results";
   ...
   begin
      ...
      Setup_Contextual_Menu (Self.Kernel, Event_On_Widget => Self.View);
   ...
   function Is_Parent_Selected
     (Selection : Gtk.Tree_Selection.Gtk_Tree_Selection;
      Path      : Gtk_Tree_Path;
      Depth     : Gint := 0) return Boolean is
   begin
      while Path.Up loop
   ...
   procedure On_Location_Selection_Changed
     (Object : access Glib.Object.GObject_Record'Class)
   is
      Self : Location_View_Record'Class renames
        Location_View_Record'Class (Object.all);
   ...
   begin
      if File /= No_File then
         GPS.Editors.GtkAda.Get_MDI_Child
           (Self.Kernel.Get_Buffer_Factory.Get (File).Current_View)
           .Raise_Child;
      end if;
   end On_File_Clicked;
   ...
   begin
      declare
         Mark    : constant Editor_Mark'Class :=
           Get_Mark
             (Gtk.Tree_Model."-" (Self.View.Get_Model),
              Iter,
              -Node_Mark_Column);
         ...
         File    : constant Virtual_File :=
           Get_File
             (Gtk.Tree_Model."-" (Self.View.Get_Model), Iter, -File_Column);
   ...
   function Get_Or_Create_Location_View
     (Kernel         : access Kernel_Handle_Record'Class;
      Allow_Creation : Boolean := True) return Location_View_Access is
   begin
      if Allow_Creation then
         ...
         return Location_View_Access (Location_Views.Retrieve_View (Kernel));
      end if;
   ...
   begin
      if Command = "parse" then
         ...
         declare
            File   : constant Virtual_File :=
              Get_Data (Nth_Arg (Data, 2, Get_File_Class (Get_Kernel (Data))));
               ...
               Ignore :=
                 GPS.Kernel.Messages.Tools_Output.Add_Tool_Message
                   (Get_Messages_Container (Get_Kernel (Data)),
                    ...
                    Get_Style_Manager (Get_Kernel (Data)).Get
      ...
      then
         ...
         if Iter /= Null_Iter then
            ...
            View.Kernel.Messages_Window.Insert
              (-"The Locations view is empty: nothing to do", Mode => Error);
            Free_Path_List (List);
            return Commands.Failure;
         end if;
      end if;
