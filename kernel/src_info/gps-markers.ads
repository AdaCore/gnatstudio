...
package GPS.Markers is
   ...
   function Go_To (Self : not null access Location_Marker_Data) return Boolean
   is abstract;
   ...
   function To_String
     (Self : not null access Location_Marker_Data) return String
   is abstract;
   ...
   function Save
     (Self : not null access Location_Marker_Data) return XML_Utils.Node_Ptr
   is abstract;
   --  Saves the marker to an XML node, so that it can be reloaded later on,
   --  possibly in a different GNAT Studio session.

   procedure Save
     (Self : not null access Location_Marker_Data; Value : out JSON_Value)
   is abstract;
   ...
   procedure Free (Self : in out Location_Marker_Data'Class)
   with Inline;
   --  Destroy self and reclame memory.

   package Markers is new
     Shared_Pointers
   ...
   is (if Left = No_Marker or else Right = No_Marker
       then Integer'Last
       else Left.Unchecked_Get.Distance (Right.Unchecked_Get));
   --  Convenience functions

   procedure Save (Self : Location_Marker; Value : out JSON_Value);
