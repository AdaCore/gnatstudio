...
package body Projects is
   ...
   begin
      if View = Prj.No_Project then
         ...
         --  ??? note that including_libraries is not taken into account
         --  in case an ada_objects_path has already been computed, and
         --  can compromise the project cache if not. So we don't use it here.
            return Prj.Env.Ada_Objects_Path
             (View, Project.View_Tree).all;
