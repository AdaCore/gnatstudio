...
package body GPS.Scripts is
   ...
   begin
      ...
      while Has_Element (C) loop
         Unset_Data
           (Element (Self.Instances, C), Script_Proxy'Class (Self).Class_Name);
   ...
   package body Script_Proxies is
      ...
      type Element_Properties_Record is new Instance_Property_Record
      with record
         Element   : Element_Type;
         Ownership : Ownership_Mode;
      end record;
      overriding
      procedure Destroy (Prop : in out Element_Properties_Record);
      ...
      overriding
      procedure Destroy (Prop : in out Element_Properties_Record) is
      begin
         ...
         if C = No_Class_Instance then
            ...
            C :=
              New_Instance
                (Script,
                 New_Class
                   (Script.Get_Repository,
                    (if Class_To_Create = ""
                     then Self.Class_Name
         ...
         Set_Data
           (Inst,
            ...
            Element_Properties_Record'
              (Element => Obj, Ownership => Ada_Owns_Python));
      ...
      begin
         if Data = null then
            raise No_Data_Set_For_Instance
              with
                "No Ada object associated with python "
                & P.Class_Name
                & " instance";
         end if;
      ...
      begin
         while Has_Element (C) loop
            if not Found then
               Detached :=
                 Detach (From_Instance (Element (Self.Instances, C)));
               Found := True;
            end if;
            Set_Data
              (Element (Self.Instances, C),
               Self.Class_Name,
               Element_Properties_Record'
                 (Element => Detached, Ownership => Python_Owns_Ada));
            Next (Self.Instances, C);
         end loop;
