...
package body Language is
   ...
   ---------------------------------
   -- Get_Parse_Entities_Iterator --
   ---------------------------------

   function Get_Parse_Entities_Iterator (Buffer : String)
                                         return Parse_Entities_Iterator is
   begin
      return (Line     => 1,
              Column   => 1,
              Index    => Buffer'First,
              Last_Idx => Buffer'Last);
   end Get_Parse_Entities_Iterator;

   ------------
   -- At_End --
   ------------

   function At_End (Iter : Parse_Entities_Iterator) return Boolean is
   begin
      return Iter.Index >= Iter.Last_Idx;
   end At_End;
   ...
   begin
      if Iter.Index < Iter.Last_Idx then
         Looking_At
           (Lang, Buffer, Iter.Index, Entity, Next_Char, Line_Inc, Column_Inc);
         ...
         if Line_Inc = 1 then
            Column_Inc := Iter.Column + Column_Inc - 1;
         end if;
         ...
         Dead := Callback
           (Entity,
            (Iter.Line, Iter.Column, Iter.Index),
            (Iter.Line + Line_Inc - 1, Col, End_Char),
            Entity = Comment_Text and then Next_Char > Buffer'Last);

         Iter.Line := Iter.Line + Line_Inc - 1;
         Iter.Column := Column_Inc;

         Iter.Index := Next_Char;
      end if;
   end Parse_Entities;
